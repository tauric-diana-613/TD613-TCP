...
5. If you are starting with your own cadence, stay in `Homebase / Personas`, paste your corpus into `Cadence Lockbox`, press `Lock`, and use `Reveal` only when you want the dossier and solo Telemetry/Harbor wake path.
6. If you want to test transformation first, stay in `Homebase / Personas`: the passage bench works before `Reveal`.
7. Choose a mask on the same page's shelf, then use `Bring into Homebase` when you want that mask to become worn instead of merely previewed.
8. In `Homebase / Personas`, read the sequence in order: `Worn Mask`, `Source`, `Through Mask`, `Before Contact`, `After Contact`, `What Moved`, `What Clung`.
...
- `Homebase / Personas` is the private cadence base and mask shelf: `Cadence Lockbox`, staged `Lock -> Reveal -> Save`, built-ins, captured shells, trained shells, worn mask, passage bench, dossier, and lock archive
...
2. the mask shelf in `Homebase / Personas` when you need to choose or dispatch a mask
...
- choosing a mask on the `Homebase / Personas` shelf only selects it there
...
- route changes such as `#console`, `#homebase`, `#readout`, `#deck`, and `#trainer` change the station shell
