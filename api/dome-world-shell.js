...
export const ASH_MASS_EVICTION_EPOCH = 'td613.ash.cache-flush/2026-07-20-aia3-mass-eviction-v2';
...
function cachePreflightBoot() {
  ...
  (()=>{
    ...
    const markerKey='td613.ash.cache-preflight.epoch';
    const receiptKey='td613.ash.cache-preflight.receipt';
    let marker=null;
    try{marker=localStorage.getItem(markerKey)}catch{}
    ...
    window.__td613AshAia3Preflight=(async()=>{
      const pointer=(()=>{try{return localStorage.getItem('td613.ash-keep.current-case')}catch{return null}})();
      const session=(()=>{try{return localStorage.getItem('td613.ash.session.epoch')}catch{return null}})();
      ...
      try{const url=new URL('/api/dome-world-shell',location.href);url.searchParams.set('surface','cache-evict');url.searchParams.set('epoch',epoch);url.searchParams.set('asset_epoch',assetEpoch);url.searchParams.set('nonce',crypto.randomUUID?.()||String(Date.now()));const response=await fetch(url,{cache:'no-store',credentials:'same-origin',headers:{'Cache-Control':'no-cache, no-store, max-age=0',Pragma:'no-cache'}});http={attempted:true,observed:response.ok,status:response.status,clear_site_data:response.headers.get('Clear-Site-Data')}}catch(error){http={attempted:true,observed:false,reason:String(error?.message||error)}}
      try{localStorage.setItem(markerKey,epoch)}catch{}
      const receipt={schema:'td613.ash.cache-preflight-receipt/v0.1',epoch,asset_epoch:assetEpoch,performed:true,http_cache:http,cache_names:cleared,worker_scopes:workers,indexeddb_preserved:true,case_data_preserved:true,active_session_reset:false,local_case_pointer_preserved:(()=>{try{return localStorage.getItem('td613.ash-keep.current-case')===pointer}catch{return false}})(),session_epoch_preserved:(()=>{try{return localStorage.getItem('td613.ash.session.epoch')===session}catch{return false}})()};
      try{sessionStorage.setItem(receiptKey,JSON.stringify(receipt))}catch{}
      const current=new URL(location.href);if(current.searchParams.get('ash_epoch')!==assetEpoch){current.searchParams.set('ash_epoch',assetEpoch);location.replace(current.pathname+current.search+current.hash);return new Promise(()=>{})}
      document.documentElement.dataset.ashCachePreflight='complete';return receipt;
    })();
  })();
  </script>`;
}
...
export function injectAshKeepLifecycle(source = '') {
  ...
  for (const [sourceModule, versionedModule] of ASH_VERSIONED_MODULES) {
    if (html.includes(versionedModule)) continue;
    ...
    html = html.replace(unversionedTag, `src="${versionedModule}"`);
  }
