...
function accessReceipt(body, session, outcome = 'COMPLETED', extra = {}) {
  return {
    ...
    request_digest: body?.request_id && body?.operation
...
async function handleAccessOperation(req, res, body, session, fetchImpl) {
  ...
  if (!enabled && session?.role !== SESSION_ROLES.OWNER) {
    return send(res, 200, {
      ok: true,
      data: { ...data, current_session_closed: true },
      receipt: accessReceipt(body, session, 'COMPLETED', { collaborator_sessions_evicted: true }),
      error: null
    }, { ...cors, 'Set-Cookie': clearSessionCookie() });
  }
