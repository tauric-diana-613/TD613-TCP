...
const VERSION='strict-endpoint-pr183.1-density-reroute-register-lane';
const META='pr183.1-density-reroute-register-lane/v1';
...
function lowSigRich(c={}){const tier=s(c.packetTier||c.flightPacket?.packetTier||c.flightPacket?.packet_tier);const st=s(c.maskEvidenceState||c.flightPacket?.maskEvidenceState);return /low_signature/i.test(tier)&&/rich/i.test(st)}
...
function weakDensityCandidate(payload={},contract={}){const candidates=arr(payload.candidates);const sourceWords=words(source(contract)).length;if(!candidates.length||sourceWords<120)return false;const ratioFloor=sourceWords>=260?0.58:0.56;const weakLength=candidates.some((candidate)=>words(ctext(candidate)).length/Math.max(1,sourceWords)<ratioFloor);const logistics=candidates.some((candidate)=>/\b(next steps|keep this organized|nobody has to repeat|for the record|the main point is|just saying|this is basically about)\b/i.test(ctext(candidate)));const placeholders=candidates.some((candidate)=>arr(candidate.authorship_moves||candidate.authorshipMoves).some((move)=>/^specific (mask|register|cadence) move$/i.test(s(move))));const summarySurface=candidates.some((candidate)=>/\b(summary|summarize|recap|main point|record note|administrative rewrite)\b/i.test(`${s(candidate.style_note||candidate.styleNote)} ${s(candidate.mask_surface_notes?.coverage)} ${s(candidate.mask_surface_notes?.structure)}`));return weakLength||logistics||placeholders||summarySurface}
function timeoutResult(timeoutMs=0,timeoutReason='strict_upstream_timeout'){return{response:{ok:false,status:408},payload:{ok:false,provider:'gemini-strict',model:timeoutReason==='strict_initial_upstream_timeout'?'strict-server-watchdog':timeoutReason,error:timeoutReason,reason:timeoutReason,warnings:[timeoutReason==='strict_initial_upstream_timeout'?'strict-server-watchdog-timeout':`${timeoutReason}-timeout`],attempts:[],requestReceipt:{strictServerTimeout:timeoutReason==='strict_initial_upstream_timeout',retryTimeoutMs:timeoutMs,localUpstreamInvoke:true,lazyLocalImport:true,weakDensityReroute:true}}}}
async function loadGenerateHandler(){const mod=await import('./hush-generate.js');return mod.default||mod.generateHandler||mod.handler||mod}
function localGenerate(req,contract){return new Promise((resolve)=>{let done=false;const headers={};function finish(status,payload){if(done)return;done=true;const code=Number(status)||200;resolve({response:{ok:code>=200&&code<300,status:code,headers},payload:payload||{}})}const res={statusCode:200,setHeader(k,v){headers[String(k).toLowerCase()]=v;return this},status(code){this.statusCode=Number(code)||200;return this},json(payload){finish(this.statusCode,payload);return payload}};const localReq={...req,method:'POST',query:{},body:{contract}};loadGenerateHandler().then((generateHandler)=>Promise.resolve(generateHandler(localReq,res))).then((value)=>{if(!done&&value!==undefined)finish(res.statusCode,value)}).catch((error)=>finish(502,{ok:false,provider:'gemini-strict',model:'local-generate-exception',error:'local-generate-exception',warnings:['local-generate-exception'],proxyError:String(error?.message||error),requestReceipt:{localUpstreamInvoke:true,lazyLocalImport:true,localGenerateException:true,weakDensityReroute:true}}))})}
async function upstream(req,contract,timeoutMs=0,timeoutReason='strict_upstream_timeout'){const request=localGenerate(req,contract);if(!(timeoutMs>0))return request;const timeout=new Promise((resolve)=>setTimeout(()=>resolve(timeoutResult(timeoutMs,timeoutReason)),timeoutMs));return Promise.race([request,timeout])}
...
function densityRerouteContract(c={}){const style=c.flightPacket?.style_diversity_policy||{};const vector=c.flightPacket?.mask_style_vector||{};const surface=s(style.surface||vector.display_name||'');const n={...c,reroll:true,candidateCount:3,packetTier:'register_transform_interpretive_density_packet',maskEvidenceState:'detailed',strictDensityReroute:true,strictDensityRerouteReason:'weak-candidate-density-reroute',strictReviewRetryAttemptBudget:3,strictReviewRetryStageLimit:2};if(c.flightPacket)n.flightPacket={...c.flightPacket,packetTier:'register_transform_interpretive_density_packet',packet_tier:'register_transform_interpretive_density_packet',maskEvidenceState:'detailed',style_diversity_policy:{...style,surface:[surface,DENSITY_LAW].filter(Boolean).join(' | '),architecture:[s(style.architecture), 'preserve hinge logic, metaphor pressure, and causal architecture'].filter(Boolean).join(' | ')},mask_style_vector:{...vector,diction_hints:uniq([...(arr(vector.diction_hints)), 'hinge logic','metaphor pressure','interpretive density','authorial posture']).slice(0,12)},flight_controls:{...(c.flightPacket.flight_controls||{}),candidate_count:3}};return n}
function meta(payload={},contract={},startedAt=Date.now(),extra=[]){return{...(payload.requestReceipt||{}),strict:true,noFallback:true,strictDirect:true,strictDirectVersion:VERSION,providerVersion:VERSION,reviewVersion:META,endpointMetaVersion:META,localUpstreamInvoke:true,lazyLocalImport:true,reviewMapSignalGate:true,weakDensityReroute:true,strictReviewMapTransformRetry:extra.includes('review-map-transform-retry-attempted')||extra.includes('review-map-transform-retry-success'),strictReviewMapLateRetry:extra.includes('review-map-transform-late-retry-attempted')||extra.includes('review-map-transform-late-retry-success'),lowSignatureRichRetry:extra.includes('low-signature-rich-retry-attempted')||extra.includes('low-signature-rich-retry-success'),strictDensityReroute:extra.includes('weak-candidate-density-reroute-attempted'),strictDensityRerouteSuccess:extra.includes('weak-candidate-density-reroute-success'),strictDensityRerouteStillHeld:extra.includes('weak-candidate-density-reroute-still-held'),packetTier:s(contract.packetTier||''),maskEvidenceState:s(contract.maskEvidenceState||''),elapsedMs:Date.now()-startedAt}}
function timeoutHold(payload={},contract={},startedAt=Date.now()){return attachStrictReceiptMeta({ok:false,status:'held',held:true,released:false,provider:'gemini-strict',model:'strict-server-watchdog',strict:true,noFallback:true,fallbackReleased:false,outputReleased:false,error:'strict_server_watchdog_timeout',reason:'strict_server_watchdog_timeout',candidates:[],warnings:uniq([...(payload.warnings||[]),'strict-server-watchdog-timeout','strict-api-no-usable-candidates','no-local-fallback']),attempts:payload.attempts||[],providerErrorMessage:'Strict endpoint stopped the local upstream handler inside the client-safe window before the page watchdog could fire.',requestReceipt:meta(payload,contract,startedAt)},contract,startedAt)}
...
export default async function handler(req,res){
  ...
  if(req.method==='GET')return send(res,200,{ok:true,route:'hush-generate-strict',version:VERSION,reviewVersion:META,upstream:'local:/api/hush-generate',lazyLocalImport:true,reviewMapSignalGate:true,weakDensityReroute:true,densityRerouteRegisterLane:true,note:'Strict endpoint reroutes weak long-form candidates into the register-transform density lane while preserving review-map containment and no-fallback behavior.'});
  ...
  try{
    ...
    if(releasable(payload)&&weakDensityCandidate(payload,contract)){
      ...
      if(remaining>=1800){
        ...
        payload={...retry.payload,warnings:uniq([...(retry.payload.warnings||[]),...densityExtra,'weak-candidate-density-reroute-still-held'])};
      }else payload={...payload,warnings:uniq([...(payload.warnings||[]),...densityExtra,'weak-candidate-density-reroute-still-held'])};
