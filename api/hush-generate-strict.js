...
const STRICT_PROVIDER_VERSION = 'hush-generate-strict-v7-wrapper-condition';
...
function compactPacket(packet = {}, candidateCount = STRICT_CANDIDATE_COUNT) {
  ...
  return {
    ...
    flight_controls: {
      ...
      wrapper_condition_required: true
    }
  };
}
...
function wrapperCondition(contract = {}, units = []) {
  return `WRAPPER CONDITION:\n- A transform may add a new opening or a new closing only when the body itself is dynamically rebuilt.\n- Prepending a new sentence to a mostly source-shaped body is not a successful transformation.\n- Appending a new sentence to a mostly source-shaped body is not a successful transformation.\n- A unique semantic wrapper cannot carry the candidate if the middle remains weak, same-order, same-syntax, or source-residual heavy.\n- Every candidate must change the internal sentence architecture, clause order, transition logic, and cadence pressure, not merely frame the source with new text.\n- If you use an opening or closing wrapper, describe in mask_surface_notes.structure how the interior body changed.\n- Candidates that rely on wrapper-only transformation should be discarded before returning JSON.`;
}
