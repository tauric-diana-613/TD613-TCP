...
function densityRerouteContract(c={}){const n={...c,reroll:true,candidateCount:3,packetTier:'interpretive_density_packet',maskEvidenceState:'detailed',strictDensityReroute:true,strictDensityRerouteReason:'weak-candidate-density-reroute',strictReviewRetryAttemptBudget:3,strictReviewRetryStageLimit:2};if(c.flightPacket)n.flightPacket={...c.flightPacket,packetTier:'interpretive_density_packet',packet_tier:'interpretive_density_packet',maskEvidenceState:'detailed',flight_controls:{...(c.flightPacket.flight_controls||{}),candidate_count:3}};return n}
...
export default async function handler(req,res){
  ...
  if(req.method==='GET')return send(res,200,{ok:true,route:'hush-generate-strict',version:VERSION,reviewVersion:META,upstream:'local:/api/hush-generate',lazyLocalImport:true,reviewMapSignalGate:true,weakDensityReroute:true,note:'Strict endpoint reroutes weak long-form candidates before release while preserving review-map containment and no-fallback behavior.'});
