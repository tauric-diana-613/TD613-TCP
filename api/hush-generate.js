...
const VERSION = 'hush-generate-v3.21.2-label-safe-functional-map';
const ROTATION_VERSION = 'pr187.2-label-safe-functional-map/v1';
...
function functionalMap(contract = {}, sourceText = '', cx = {}) {
  ...
  return [...buckets.entries()].map(([key, values]) => `${key}: ${uniqText(values).slice(0, 3).join(' || ') || '(no required item)'}`);
}
...
function macroMismatch(candidate = {}) {
  ...
  const normalized = declared.includes('risk') ? 'risk' : declared.includes('mechanism') ? 'mechanism' : declared.includes('practical') ? 'practical-pressure' : declared.includes('uncertainty') ? 'uncertainty' : declared.includes('release') ? 'release-dilemma' : declared;
  return { mismatch: normalized !== detected, declared: normalized, detected };
}
function labelLeak(text = '') { return { leaked: ROUTE_LABEL_PATTERN.test(safe(text)) }; }
...
function splitCandidates(candidates = [], sourceText = '', cx = {}, contract = {}) {
  ...
  candidates.forEach((candidate, index) => {
    ...
    if (leak.leaked) copied.push({ index, risk: { copied: true, routeLabelLeak: true }, preview: safe(text).slice(0, 180) });
...
function buildPrompt(contract = {}, repair = null) {
  ...
  const custody = `FUNCTIONAL CUSTODY MAP — NOT SOURCE ORDER:\n${groups.map((group) => `- ${group}`).join('\n') || '- none'}\nSEMIOTIC ANCHORS: ${anchors.join(', ') || '(none)'}\nPROTECTED LITERALS: ${literals.join(', ') || '(none)'}`;
  const sourceBlock = deSource ? `${deSource === 'repair' ? 'DE-SOURCED COPY REPAIR MODE' : 'FIRST-PASS DE-SOURCE MODE'}:\nThe raw source text is withheld. Rebuild from this functional custody map only. The map is grouped by function, not sequence. Do not recreate the original opening, order, or phrasing.\n${custody}` : `${custody}\n\nMESSAGE TO TRANSFORM:\n${sourceText}`;
...
${(cx.sourceRegisterOverlap || cx.lowSignatureCadence || cx.firstPassDeSource) ? 'NO STYLE COSTUME LAW: The source already contains casual or low-signature diction. Recompose from functional custody groups. Do not use the map as sequence.' : ''}
${(cx.sourceRegisterOverlap || cx.lowSignatureCadence || cx.firstPassDeSource) ? 'MACRO-ORDER REQUIREMENTS: Candidate 1 begins from real practical constraints, not the phrase practical-pressure. Candidate 2 begins from the actual validation mechanism, not the phrase validation-mechanism. Candidate 3 begins from the actual release risk, not the phrase release-risk. If a fourth candidate exists, begin from the actual uncertainty/observation pressure.' : ''}
${maskFailureLaw(contract)}

ROUTE LABELS ARE METADATA ONLY: Never put these strings in candidate text: practical-pressure, validation-mechanism, release-risk, uncertainty-observation, seed-state, residual-context, functional custody, macro_start. Start with the actual content, never the map label.
ORDERED CUSTODY MAP FORBIDDEN: The functional custody map is a bag of obligations, not an outline. Choose a new route and declare it in macro_start.
...
For macro_start, name the candidate's actual first route. For authorship_moves, name actual moves such as "rebuilt from functional custody groups," "recomposed away from source sequence," or "kept the technical mechanism visible while changing the route."
...
function normalizeCandidates(value) {
  ...
  return source.map((candidate, index) => {
    ...
    return text ? {
      ...
      authorship_moves: stringArray(candidate.authorship_moves || candidate.authorshipMoves).length ? stringArray(candidate.authorship_moves || candidate.authorshipMoves) : ['rebuilt from functional custody groups', 'recomposed away from source sequence'],
