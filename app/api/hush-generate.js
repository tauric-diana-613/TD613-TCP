...
async function fetchModelInventory({ force = false } = {}) {
  if (!process.env.GEMINI_API_KEY) return { ok: false, status: 501, error: 'remote-llm-proxy-not-configured', models: [], generateContentModels: [] };
  const now = Date.now();
...
async function resolveModels({ forceInventory = false } = {}) {
  ...
  const models = inventory.ok ? sortModels([...overrides, ...inventoryModels]) : sortModels([...overrides, ...DEFAULT_GEMINI_MODELS]);
...
async function callGemini({ model, prompt, jsonMode }) {
  ...
  const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/' + encodeURIComponent(normalizeModelName(model)) + ':generateContent?key=' + encodeURIComponent(process.env.GEMINI_API_KEY), {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }] }], generationConfig })
  });
  const payload = await response.json().catch(() => ({}));
...
function queryFlags(req) {
  try {
    ...
    return { probe: url.searchParams.has('probe') || url.searchParams.has('selftest'), models: url.searchParams.has('models') || url.searchParams.has('listModels'), refresh: url.searchParams.has('refresh') || url.searchParams.get('cache') === 'bust' };
  } catch { return { probe: false, models: false, refresh: false }; }
}
