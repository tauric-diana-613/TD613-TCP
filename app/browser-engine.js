(function () {
  ...
  function buildCadenceTransfer(text = '', shell = {}, options = {}) {
    ...
    const runCandidate = (spec = {}) => {
      ...
      return {
        ...
        debug: candidateDebugPasses
          ? {
              passes: candidateDebugPasses.slice(),
    ...
    const bestCandidate = candidates.slice().sort((left, right) => right.score - left.score)[0];
    ...
    let changedDimensions = bestCandidate.changedDimensions.slice();
    ...
    if (debug) {
      result.debug = {
        ...
        candidates: candidates.map((candidate) => ({
          ...
          changedDimensions: candidate.changedDimensions.slice(),
          qualityGatePassed: candidate.quality.qualityGatePassed,
          notes: candidate.quality.notes.slice(),
          passesApplied: candidate.passesApplied.slice()
        })),
