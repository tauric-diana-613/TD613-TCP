...
function buildSwapLaneResult(sourceSample, donorSample, slot = 'A', donorSlot = 'B', strength = 0.82) {
  ...
  return {
    ...
    borrowedShellOutcome:
      transfer.borrowedShellOutcome ||
      (transfer.holdStatus === 'held' ? 'rejected' : (transfer.transferClass === 'rejected' ? 'rejected' : 'subtle')),
    borrowedShellFailureClass:
      transfer.borrowedShellFailureClass ||
      (transfer.holdStatus === 'held' ? transfer.generationDocket?.holdClass || 'below-rewrite-bar' : null),
