(async function () {
  ...
  function normalizeGatewayApertureBridgeSummary(payload = {}) {
    ...
    return {
      ...
      mode: 'gateway-embed',
      type: 'status',
      apertureVersion: String(payload.apertureVersion || payload.version || latestPacket?.apertureVersion || latestPacket?.version || 'v3.2-alpha'),
      apertureSchema: String(payload.apertureSchema || payload.schemaVersion || latestPacket?.apertureSchema || latestPacket?.schemaVersion || 'td613-aperture/v3.2-alpha'),
      apertureFeatureVersion: String(payload.apertureFeatureVersion || latestPacket?.apertureFeatureVersion || 'v3.2-alpha-typed-epistemic-deficit-and-stability-aware-widening-runtime'),
      eorfdOperationalState: String(payload.eorfdOperationalState || latestPacket?.eorfdOperationalState || 'interface_context'),
      eorfdClaimAuthority: String(payload.eorfdClaimAuthority || latestPacket?.eorfdClaimAuthority || 'design_signal'),
      eorfdTargetState: String(payload.eorfdTargetState || latestPacket?.eorfdTargetState || 'verified-runtime-installation'),
      domeWorldVersion: String(payload.domeWorldVersion || latestPacket?.domeWorldVersion || 'v0.4.3'),
      domeWorldReceiptRef: payload.domeWorldReceiptRef || latestPacket?.domeWorldReceiptRef || null,
  ...
  function updateSafeHarborEntryPoints({
      ...
      state === 'ready'
        ...
        : state === 'guarded'
          ...
          : state === 'branch'
            ...
            : 'Safe Harbor remains available as a separate chamber for canonical intake and packet shaping.';

    const railNode = $('safeHarborHandoff');
    const railLabelNode = $('safeHarborHandoffLabel');
    if (railNode && railLabelNode) {
      railNode.dataset.state = state;
      railNode.href = href;
      railNode.title = detail;
      railNode.setAttribute('aria-label', detail);
      railLabelNode.textContent =
        state === 'ready'
    ...
    const heroActionNode = $('heroHarborAction');
    if (heroActionNode) {
      const showHeroAction = state !== 'latent';
      heroActionNode.hidden = !showHeroAction;
      heroActionNode.dataset.state = state;
      heroActionNode.href = href;
      heroActionNode.title = detail;
      heroActionNode.textContent =
        state === 'ready'
          ? 'Enter Safe Harbor'
          : state === 'guarded'
            ? 'Open Guarded Safe Harbor'
            : 'Stage In Safe Harbor';
    }
  }
