export const ASH_DEMO_PEDAGOGY_VERSION = 'td613.ash.demo-pedagogy/v0.1-four-profile-rehydration';
...
function ensureStyles() {
  ...
  style.textContent = `
    .ash-demo-pedagogy{margin:0 0 18px;padding:18px;border:1px solid rgba(118,234,212,.26);background:linear-gradient(145deg,rgba(4,20,16,.97),rgba(18,12,27,.88));box-shadow:0 18px 48px rgba(0,0,0,.2)}
    .ash-demo-pedagogy header{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:start}
    .ash-demo-pedagogy h3{margin:0;font:500 clamp(1.35rem,2.6vw,2.15rem)/1.08 var(--serif)}
    .ash-demo-pedagogy p{color:var(--muted);line-height:1.55}
    .ash-demo-pedagogy__chip{padding:7px 9px;border:1px solid rgba(228,198,108,.38);color:var(--gold);font:700 .58rem/1.2 var(--mono);text-transform:uppercase;letter-spacing:.08em}
    .ash-demo-pedagogy__steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin:14px 0 0;padding:0;list-style:none}
    .ash-demo-pedagogy__steps button{width:100%;min-height:112px;padding:12px;text-align:left;border:1px solid rgba(118,234,212,.2);background:rgba(2,10,8,.72);color:var(--paper);cursor:pointer}
    .ash-demo-pedagogy__steps button:hover,.ash-demo-pedagogy__steps button:focus-visible{border-color:var(--gold);background:rgba(228,198,108,.07)}
    .ash-demo-pedagogy__steps button[data-current="true"]{border-color:var(--cyan);box-shadow:inset 0 0 0 1px rgba(118,234,212,.16)}
    .ash-demo-pedagogy__steps span{display:block;color:var(--cyan);font:700 .56rem var(--mono);text-transform:uppercase}
    .ash-demo-pedagogy__steps strong{display:block;margin:8px 0 5px;font:600 .9rem var(--mono)}
    .ash-demo-pedagogy__steps small{display:block;color:var(--muted);font:500 .68rem/1.45 var(--mono)}
    .ash-demo-pedagogy__ledger{margin-top:12px;border-top:1px solid rgba(118,234,212,.14);padding-top:10px}
    .ash-demo-pedagogy__ledger summary{cursor:pointer;color:var(--gold);font:700 .62rem var(--mono);text-transform:uppercase;letter-spacing:.08em}
    .ash-demo-pedagogy__groups{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin-top:10px}
    .ash-demo-pedagogy__group{padding:10px;border:1px solid rgba(118,234,212,.12);background:rgba(1,8,6,.58)}
    .ash-demo-pedagogy__group h4{margin:0 0 7px;color:var(--paper);font:700 .6rem var(--mono);text-transform:uppercase}
    .ash-demo-pedagogy__group ul{margin:0;padding-left:16px;color:var(--muted);font:500 .62rem/1.45 var(--mono)}
    .ash-demo-pedagogy__group li[data-state="MISSING"],.ash-demo-pedagogy__group li[data-state="DRIFT"]{color:var(--rose)}
    .ash-demo-pedagogy__ceiling{margin:12px 0 0!important;color:var(--rose)!important;font:700 .62rem var(--mono)!important;text-transform:uppercase}
    @media(max-width:860px){.ash-demo-pedagogy__steps{grid-template-columns:repeat(2,minmax(0,1fr))}.ash-demo-pedagogy__groups{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:520px){.ash-demo-pedagogy{padding:14px}.ash-demo-pedagogy header{grid-template-columns:1fr}.ash-demo-pedagogy__steps,.ash-demo-pedagogy__groups{grid-template-columns:1fr}.ash-demo-pedagogy__steps button{min-height:88px}}
  `;
...
function inspectSurface(item, expected) {
  ...
  let state = node ? 'PRESENT' : 'MISSING';
...
function group(label, items) {
  return `<section class="ash-demo-pedagogy__group"><h4>${label}</h4><ul>${items.map(item => `<li data-state="${item.state}" title="${item.reason}">${item.label} · ${item.state.replaceAll('_',' ').toLowerCase()}</li>`).join('')}</ul></section>`;
}

function currentProfile() {
  const explicit = doc.documentElement.dataset.ashDemoProfile;
  if (ASH_DEMO_PEDAGOGY_MANIFESTS[explicit]) return explicit;
  const selected = byId('newProfile')?.value;
  return ASH_DEMO_PEDAGOGY_MANIFESTS[selected] && host.localStorage?.getItem?.('td613.ash-keep.current-case') ? selected : null;
}
...
function openWorkspace(name) {
  ...
  open(name);
  host.dispatchEvent(new CustomEvent('td613:ash:demo-pedagogy-step', { detail:{ profile:currentProfile(), workspace:name, version:ASH_DEMO_PEDAGOGY_VERSION } }));
  return true;
}
...
function decorateDestinations(manifest) {
  doc.querySelectorAll('[data-premium-workspace]').forEach(button => {
    const workspace = button.dataset.premiumWorkspace;
    const purpose = manifest.destination_copy[workspace];
    if (!purpose) return;
    button.dataset.demoProfile = manifest.profile;
    button.dataset.demoPurpose = purpose;
    button.title = purpose;
    button.setAttribute('aria-description', purpose);
  });
}
...
function render(manifest) {
  ...
  if (!panel) {
    ...
    panel.id = 'ashDemoPedagogyLedger';
    panel.className = 'ash-demo-pedagogy';
  ...
  const title = byId('ashAiaTitle');
  if (title) title.textContent = manifest.title;
  const consequence = doc.querySelector('[data-aia-consequence]');
  if (consequence) consequence.textContent = manifest.consequence;
  updateMotionLabels(manifest);
  decorateDestinations(manifest);
  return audit;
}
...
function schedule() {
  ...
  host.setTimeout(() => {
    ...
    if (!profile) return;
    const manifest = ASH_DEMO_PEDAGOGY_MANIFESTS[profile];
    const audit = render(manifest);
    host.__td613AshDemoPedagogyState = Object.freeze({ profile, manifest, audit });
    host.dispatchEvent(new CustomEvent('td613:ash:demo-pedagogy-hydrated', { detail:{ profile, version:ASH_DEMO_PEDAGOGY_VERSION, missing:audit?.missing || [], drift:audit?.drift || [] } }));
  }, 0);
}
...
export function installAshDemoPedagogy(docArg = document, hostArg = window) {
  ...
  ensureStyles();
  docArg.addEventListener('click', event => {
    const button = event.target?.closest?.('[data-demo-pedagogy-workspace]');
    if (!button) return;
