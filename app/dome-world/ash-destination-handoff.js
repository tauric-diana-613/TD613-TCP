...
const setState = (label, ok = false) => {
  $('state').textContent = label;
  $('state').className = `status ${ok ? 'ok' : 'hold'}`;
};
...
function requestRecipientReady() {
  const frame = $('recipientFrame');
  if (!frame?.contentWindow || state.recipientReady) return;
  frame.contentWindow.postMessage({ type: 'TD613_ASH_RECIPIENT_PING' }, location.origin);
}
...
function deliverPacket(packet) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      channel.port1.close();
      reject(new Error('Named recipient timed out.'));
    }, 8000);
...
async function send() {
  ...
  if (!state.recipientReady) {
    requestRecipientReady();
    setState('RECIPIENT_HOLD · named recipient readiness not observed');
    return;
  }
  ...
  if (state.authorization.state !== 'DESTINATION_HANDOFF_AUTHORIZED') {
    setState(state.authorization.state);
    render();
    return;
  }
  try {
    ...
    if (state.attempt.outcome !== 'DELIVERED' || state.recipientReceipt.state !== 'RECIPIENT_RECEIPT_VERIFIED') {
      state.rollback = await compileDestinationHandoffRollback({
        attempt: state.attempt,
    ...
    state.accounting = await compileDestinationHandoffCustodyAccounting({
      attempt: state.attempt,
      recipientReceipt: state.recipientReceipt,
      rollback: state.rollback,
      whatLeft: state.attempt.what_left,
      whatRemained: state.attempt.what_remained
    });
...
window.addEventListener('message', event => {
  ...
  state.recipientReady = event.data.recipient_id === $('recipientId').value
    && event.data.destination_id === $('destinationId').value;
  if (state.recipientReady) clearInterval(readinessInterval);
});

const readinessInterval = setInterval(requestRecipientReady, 250);
$('recipientFrame').addEventListener('load', requestRecipientReady);
...
queueMicrotask(requestRecipientReady);

window.__td613AshDestinationHandoff = Object.freeze({
  version: 'td613.ash.destination-handoff.browser/v0.2-reciprocal-readiness',
  current: () => ({
    state: $('state').textContent,
    recipient_ready: state.recipientReady,
    accounting: state.accounting,
    replay: state.replay
  }),
  requestRecipientReady,
