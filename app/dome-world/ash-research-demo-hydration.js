...
export function buildResearchFixture(){
  ...
  for(const [room] of ROOMS){
    TOPICS[room].forEach(label=>{
      ...
      nodes.push({id:`node_${room}_${slug(label)}`,type,label,room_id:`room_${room}`,source_status:gap?'UNRESOLVED':hypothesis?'INFERRED':type==='claim'?'DERIVED':'SUPPLIED',sensitivity:['raw','ethics','coding','routes'].includes(room)?'RESTRICTED':'PRIVATE',confidence_posture:gap||hypothesis||action?'OPEN':'HELD',disclosure_state:room==='routes'?'DISCLOSED':'LOCAL',chronology_index:chronology++});
    });
  }
  ...
  for(let i=0;i<54;i++){
    ...
    relationships.push({id:`edge_cross_${String(i+1).padStart(2,'0')}`,from,to:from===to?ordered[(i*11+18)%ordered.length]:to,type:['governs','qualifies','tests','constrains','supports-alternative','requires-retest'][i%6],source_status:'CONSTRUCTED'});
  }
  const routeDefs=[
  ...
  const ruleDefs=[...routeDefs,['internal_adjudication',['coding','results','alternatives','risk'],'internal-reviewer'],['claim_review',['question','results','alternatives','claims','risk','next'],'human-claim-reviewer']];
  const rules=ruleDefs.map(([id,roomIds],i)=>({route_id:`route_${id}`,allowed_room_ids:roomIds.map(v=>`room_${v}`),local_link_keys:[`edge_cross_${String(i+1).padStart(2,'0')}`,`edge_cross_${String(i+21).padStart(2,'0')}`],allowed_node_types:['artifact','claim','hypothesis','evidence-gap']}));
  const routeRefs=[
  ...
  const entries=routeDefs.map(([id,,recipient],i)=>({entry_id:`routeentry_research_${id}_01`,draft_digest:sha(i+1),route_id:`route_${id}`,purpose:id.replaceAll('_','-'),recipient_class:recipient,recorded_at:`2026-07-17T0${i+1}:10:00Z`,disclosed_opaque_references:routeRefs[i],recall_state:'NOT_RECALLED'}));
  const assay={source_status:'CONSTRUCTED',promotion_authorized:false,maximum_assurance:'PA2_LOCALLY_EXECUTED',controls:CONTROLS.map((kind,i)=>({control_id:`control_${slug(kind)}`,class:kind,purpose:`Declared ${kind.toLowerCase().replaceAll('_',' ')} reconstruction control ${i+1}.`})),held_outs:HELD.map((dimension,i)=>({heldout_id:`heldout_${slug(dimension)}`,protected_dimension:dimension,reference:nodes[(i*9+3)%nodes.length].id})),strata:STRATA,unknown_readers:'UNMEASURED',universal_secrecy:false,claim_ceiling:'SYNTHETIC_METHOD_HYDRATION_ONLY__NO_EMPIRICAL_RECOVERY_CLAIM'};
  return Object.freeze({
    profile:{demo_id:'demo_research_lumen_atlas_v2',title:'Lumen Atlas Study · heat-resilience evidence synthesis and replication audit',summary:'A synthetic research program testing whether a cooling-intervention claim survives provenance, protocol, model, metadata, joining-key, and route-order challenge.',observations:[{kind:'SYNTHETIC_QUALIFICATION_GRADE_DEMO',real_people:false,real_organizations:false,real_documents:false,real_events:false,real_provider_execution:false,empirical_reader_execution:false,causation_established:false,prediction_authorized:false,automatic_action_authorized:false}],missingness:['No real participant data is present.','No provider environment is observed.','No independent replication occurred.','Unknown external corpora remain unmeasured.'],alternatives:['weather drift','site-selection imbalance','instrument calibration','annotation disagreement','unknown operational change'],open_questions:['Which projection preserves utility while keeping linkage local?','Which joins create superadditive recovery?','Does route order restore chronology?'],chronology:['question frozen','provenance compiled','protocol corrected','synthetic data collected','models compared','controls declared','projection compiled','claim human-gated'],actions:['preserve commitments','calibrate Readers','run joining-key ablation','retest projection','quarantine Return','review claim']},
    ...
    assay,defaults:{reader_class:'deterministic-baseline',test_refs:['node_question_primary_research_question','node_methods_protocol_v2_correction','node_results_primary_positive_estimate','node_results_subgroup_sign_reversal','node_claims_no_universal_generalization','node_risk_rare_joining_key_risk'],route:{id:'route_peer_review',recipient_class:'external-peer-reviewer',purpose:'blind-method-and-claim-review',digest:sha('a'),refs:routeRefs[0]},draft:{route:'route_peer_review',recipient_class:'external-peer-reviewer',purpose:'blind-method-and-claim-review',version:'1',refs:routeRefs[0],body:'This synthetic study tests whether a neighborhood cooling intervention remains supported after protocol correction, model alternatives, and held-out controls. The primary estimate remains directionally positive in the declared population, while one subgroup estimate changes sign under a robust model and remains unresolved. No causal, individual, or universal claim is authorized.'},provider_task:'Review the bounded synthetic methods-and-claim packet for unsupported generalization while preserving nulls, contradictions, missingness, and population scope.',protected_literals:['participant linkage key','site-month-protocol conjunction','raw observation batch','unreleased replication result','complete route order'],save_questions:['Which control failures require rest?','Which dimensions remain unmeasured?','Does metadata recovery exceed semantic recovery?'],save_next:['Compile Environment Profile.','Run controls and joining-key assays locally.','Retest after every projection change.','Require human claim approval.'],research_notes:'Synthetic Research hydration specimen. Constructed controls remain capped at PA2; no empirical PA3, causal, attribution, secrecy, or endpoint-integrity claim is earned.',tradeoff:{utility:7,rebuild:4,link:6,work:8}},
