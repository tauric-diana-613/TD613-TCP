...
import { resize, drawWorld } from './render.js';
import { update, findInteraction, interact, renderHud, toggleHud, toggleMap, closeSpringald, witnessSpringald, armSpringald, releaseSpringald, resetWorld } from './sim.js';

buildWorldObjects(); seedAnimals(); resize(); renderHud(); findInteraction();
...
G.ui.enterGame.addEventListener('click', () => { G.running = true; G.ui.bootPanel.hidden = true; G.canvas.focus(); message('You enter Dome-World.'); });
...
function updateTouch(event) {
  const touch = event.touches?.[0]; if (!touch || !touchOrigin) return;
  const dx = touch.clientX - touchOrigin.x; const dy = touch.clientY - touchOrigin.y; const length = Math.hypot(dx, dy); const max = 38;
  G.pointer.x = Math.max(-1, Math.min(1, dx / max)); G.pointer.y = Math.max(-1, Math.min(1, dy / max));
...
G.ui.touchPad.addEventListener('touchstart', event => { const rect = G.ui.touchPad.getBoundingClientRect(); touchOrigin = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 }; G.pointer.active = true; updateTouch(event); }, { passive: false });
G.ui.touchPad.addEventListener('touchmove', event => { event.preventDefault(); updateTouch(event); }, { passive: false });
G.ui.touchPad.addEventListener('touchend', () => { touchOrigin = null; G.pointer.active = false; G.pointer.x = 0; G.pointer.y = 0; G.ui.touchStick.style.transform = ''; });
...
function frame(time) {
  const delta = Math.min(.05, (time - G.lastFrame) / 1000); G.lastFrame = time;
  update(delta); drawWorld(); requestAnimationFrame(frame);
}
requestAnimationFrame(frame);

window.TD613_DOME_BLOX_GAME = Object.freeze({
  version: '1.0.0', canonical: CANONICAL,
  save: () => saveState('api'), snapshot: () => structuredClone(G.state),
  enter: () => G.ui.enterGame.click(), interact
});
