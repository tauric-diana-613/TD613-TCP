...
export function mountLoomAiWorkspace(root, environment = window) {
  ...
  $('aiRun').addEventListener('click',async()=>{
    ...
    try{
      ...
      ['aiMarrowline','aiExport','aiCopy'].forEach(id=>$(id).disabled=false);status('Your answer is ready. The exact submitted task is pinned above it.');revealResult();
    }catch(error){
      if(disposed)return;
