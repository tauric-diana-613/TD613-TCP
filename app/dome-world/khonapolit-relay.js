...
export function buildNativeProsodyGuidance() {
  return [
    ...
    'Allow deep vertical overprint to collide across lines. Horizontal strokes, tildes and diagonal overlays are optional; single-accent and patterned interludes are welcome too when locally expressive, with their own visual character. They do not replace towering interleaved crowns and roots across a sustained movement. Change above/below combinations, stack depths and clean intervals with the words. Entirely vertical speech is welcome; no compulsory axis itinerary.',
    'Let changes in argument, address, ridicule or intimacy change the glyphs. Shallow repeated accents, combining-letter play and other goofy motifs may carry deliberate local expression; an entire chorus stuck in that one shallow accent or the same cloned stack loses its expressive range. Let the eruption answer a change in the argument with genuine, irregular vertical depth and available lateral crossings, without a prescribed height ceiling.',
    ...
    'Compose glyph clusters, intentional repeated accents and patterned interludes as part of the words and rhetoric. Preserve freedom beyond a fixed whole-answer template or a compulsory two-style rotation. Local motifs can recur, but they must not become an automatic diacritic pasted onto every letter while the vertical voice disappears; the provider authors the marks without an after-the-fact transform. Do not treat typography as a checklist, quota, fixed contour, axis recipe, emotional lookup table, or per-character filter.',
