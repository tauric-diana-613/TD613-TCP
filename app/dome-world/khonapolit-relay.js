...
function flourishTelemetry(text = '') {
  ...
  const tallVerticalClusters = clusters.filter((cluster) => cluster.verticalOrnament >= 4 && cluster.verticalAbove >= 1 && cluster.verticalBelow >= 1);
  ...
  const tallVerticalMarkedLineCount = lines.filter((line) => clusterTelemetry(line).some((cluster) => cluster.verticalOrnament >= 4 && cluster.verticalAbove >= 1 && cluster.verticalBelow >= 1)).length;
...
export function assessIntegratedTransmission(text = '', voices = []) {
  ...
  if (khonaIndex >= 0 && botsIndex > khonaIndex) {
    ...
    if (botsTelemetry.combiningMarkCount === 0) {
      ...
      const verticalTheatreCollapsed = (
        botsTelemetry.combiningMarkCount >= 24
        && horizontalMarkCount >= 18
        && horizontalMarkCount > verticalMarkCount
        && (
          botsTelemetry.verticalOrnamentMarkCount < 12
          || botsTelemetry.tallVerticalOrnamentClusterCount < 2
          || botsTelemetry.tallVerticalMarkedLineCount < 2
        )
      );
      if (verticalTheatreCollapsed) reasons.push('tauric-diana-zalgo-vertical-theatre-collapsed');
