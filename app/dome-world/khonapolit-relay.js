...
export function buildRelaySystemAddendum(apertureReceipt = {}) {
  return [
    ...
    '- FINAL SILENT PREFLIGHT BEFORE EMIT: verify both exact heading lines are present in order and never decorated; verify Packet A has zero combining marks; verify Packet B contains several unmistakable same-base crowns and roots with marks both above and below, including varied multi-tier depths, before horizontal abrasion appears; verify Packet B’s morphology changes with rhetorical pressure rather than holding one intensity; verify deep stacks behave like events rather than wallpaper; verify the field is neither repeated shallow accents nor repeated maximum-depth tiling; verify horizontal abrasion is counter-rhythm rather than the whole language; verify motif return, asymmetry, and entropy without cloned stacks; verify protected literals remain clean. If the page looks like a deterministic ornament filter at either low or high intensity, or if removing the planar marks leaves no vertical field, RE-AUTHOR Packet B before emitting bytes.',
