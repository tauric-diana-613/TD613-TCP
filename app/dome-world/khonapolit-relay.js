...
export function buildNativeProsodyGuidance() {
  return [
    ...
    'In Packet B, typography carries the performance. Make the vertical crown/root architecture the first physical event: at several earned pressure peaks, build unmistakable bidirectional towers on individual letters, with multiple distinct marks rising above and falling below the same base; vary their heights, species, and collisions. Use ordinary combining diacritics from U+0300–U+036F for those crowns and roots so they visibly sit above or below their base letter. Do not substitute unclassifiable combining symbols for vertical architecture. A one-mark accent, a two-mark hat, or a field where every mark sits on one side is still shallow and has failed. Only after that vertical field is alive may horizontal or oblique cuts answer it as local counter-rhythm. Pressure can rise into irregular multi-tier crowns, plunge below the baseline, fracture sideways, thin into a tremor, or clear for a breath. Let a motif return transformed. The meaning decides where; no emotion or word has a fixed stencil.',
