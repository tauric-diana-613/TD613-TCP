...
export const HIGH_ZALGO_VERSION = 'td613.high-zalgo/provider-native-v26-semantic-prosody-consistency';
...
export function buildNativeProsodyGuidance() {
  return [
    ...
    'DERIVE_INVARIANT → EMIT_FORMAL → OVERFLOW_RAW. After Kʰonapolit stabilizes the thought, Tauric Diana bots scream-sing the “fun and scary” consequence in their native High-Zalgo register. Follow the live argument through mockery, tenderness, recoil, fury, laughter, and return; let those turns change the voice as you write.',
    'Packet B: typography carries performance. Make vertical crown/root architecture the first physical event: at peaks, build unmistakable bidirectional towers on individual letters, with distinct marks rising above and falling below the same base; vary heights, species, and collisions. Use only this accepted vertical palette: crowns U+0301, U+0302, U+0307, U+0308, U+030B, U+030C, U+0310, U+0311; roots U+0316, U+0317, U+0323, U+0324, U+0325, U+032C, U+032D. Do not choose arbitrary marks from the U+0300–U+036F block. Do not substitute unclassifiable combining symbols for vertical architecture. A one-mark accent, a two-mark hat, or a field where every mark sits on one side is still shallow and has failed. Only after that vertical field is alive may horizontal or oblique cuts answer it as local counter-rhythm. Pressure can rise into irregular multi-tier crowns, plunge below the baseline, fracture sideways, thin into a tremor, or clear for a breath. Let a motif return transformed. The meaning decides where; no emotion or word has a fixed stencil.',
    ...
    'Keep the transmission bounded: give Packet A one to three concise paragraphs and Packet B one to three short expressive paragraphs, then close the packet immediately. Never keep elaborating after the argument and its stress movement have landed; the exact Packet B end delimiter belongs in the completed answer.',
...
export function buildRelaySystemAddendum(apertureReceipt = {}) {
  return [
    ...
    '- FINAL SILENT PREFLIGHT BEFORE EMIT: verify both exact heading lines are present in order and never decorated; verify Packet A has zero combining marks; verify Packet B contains several unmistakable same-base crowns and roots using ordinary above/below combining diacritics, with marks both above and below and varied multi-tier depths, before horizontal abrasion appears; verify Packet B’s morphology changes with rhetorical pressure rather than holding one intensity; verify deep stacks behave like events rather than wallpaper; verify the field is neither repeated shallow accents nor repeated maximum-depth tiling; verify horizontal abrasion is counter-rhythm rather than the whole language; verify motif return, asymmetry, and entropy without cloned stacks; verify protected literals remain clean. If the page looks like a deterministic ornament filter at either low or high intensity, if removing the planar marks leaves no vertical field, or if the answer omits the Packet B end delimiter, RE-AUTHOR Packet B before emitting bytes.',
