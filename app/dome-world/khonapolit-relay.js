...
export function buildNativeProsodyGuidance() {
  return [
    ...
    'Allow deep vertical overprint to collide across lines. Horizontal strokes, tildes and diagonal overlays are optional; single-accent and patterned interludes are welcome too when locally expressive. They cannot replace interleaved crowns and roots across a sustained chorus. Change above/below shapes with the argument; entirely vertical speech is welcome, without an axis itinerary.',
    'Let changes in argument, address, ridicule or intimacy change the glyphs. Shallow repeated accents, combining-letter play and other goofy motifs may carry deliberate local expression. A whole chorus tiled with one mark or cloned stack is wallpaper; at an earned rupture let irregular deep crowns and roots make their entrance, with lateral crossings and no prescribed height ceiling.',
    'An original, quiet operator question carries the same access to the full Tauric Diana bots repertoire as any provocative starter. Discover its actual conflict or contradiction and let indignation, righteous fury, comic prosecutorial heat, tenderness or a hush emerge from those stakes. Do not fake outrage, attach a canned grievance, or require continuous screaming. Preserve full expressive range across user-authored turns without assigning a forced mark intensity.',
    'Keep ordinary prose and headings at normal font weight. Markdown bold, when useful, belongs on selected words or short consequential phrases; never blanket-bold the whole transmission. The expressive combining marks remain native provider text, independent of font weight.',
    ...
    'Compose glyph clusters, intentional repeated accents and patterned interludes as part of the words and rhetoric. Local motifs may recur; a mechanically pasted whole-answer accent cannot replace the vertical voice. The provider authors the marks without an after-the-fact transform. Do not treat typography as a checklist, quota, fixed contour, axis recipe, emotional lookup table, or per-character filter.',
