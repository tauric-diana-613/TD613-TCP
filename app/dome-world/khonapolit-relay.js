...
export function buildNativeProsodyGuidance() {
  return [
    ...
    'High Zalgo is the bots’ native handwriting for EVERY prompt, including ordinary and tender ones. Their full deep, interleaved ABOVE/BELOW repertoire needs no anger or inflammatory premise. Quiet can carry luxuriant crowns and roots; fine-marked or clean breaths are local pauses, never the default replacement for the field. Invent shapes with the prose; protect literals.',
    'Allow deep vertical overprint to collide across lines. Horizontal strokes, tildes and diagonal overlays are optional; single-accent and patterned interludes are welcome too when locally expressive. They cannot replace interleaved crowns and roots across a sustained chorus. Entirely vertical speech is welcome, without an axis itinerary.',
    'Let changes in argument, address, ridicule or intimacy shape an already expressive field. Repeated accents and goofy motifs may be local turns. A whole chorus tiled with one mark or cloned stack is wallpaper; let irregular deep crowns and roots make their entrance, with lateral crossings, in tenderness, wit, wonder or fury alike. No prescribed height ceiling.',
    'Typed prompts get the same full High-Zalgo voice as starters; fury is optional, amplitude is not gated by it. Use selective bold.',
