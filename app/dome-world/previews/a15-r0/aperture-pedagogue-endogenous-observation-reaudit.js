...
const CONTROL_OPERATOR = Object.freeze([
  Object.freeze([1, 0]),
  Object.freeze([0, 1])
]);
...
const CANDIDATES = Object.freeze([
  Object.freeze({
    candidate_id: 'Q_COLLAPSE',
    q_pre: Object.freeze([0, 1]),
    q_post: Object.freeze([1, 0]),
    transition_status: 'DECLARED_SYNTHETIC'
  }),
  Object.freeze({
    candidate_id: 'Q_FRAGILE',
    q_pre: Object.freeze([0, 1]),
    q_post: Object.freeze([1, 0.001]),
    transition_status: 'DECLARED_SYNTHETIC'
  }),
  Object.freeze({
    candidate_id: 'Q_STABLE',
    q_pre: Object.freeze([1, 1]),
    q_post: Object.freeze([0, 1]),
    transition_status: 'DECLARED_SYNTHETIC'
  }),
  Object.freeze({
    candidate_id: 'Q_UNKNOWN',
    q_pre: Object.freeze([0, 1]),
    q_post: null,
...
function normalizeRow(row, label) {
  ...
  return row.map((value, index) => finiteNumber(value, `${label}[${index}]`));
}

export function operatorGeometry(rows = []) {
  if (!Array.isArray(rows) || rows.length < 1) {
    throw new TypeError('rows must contain at least one observation row');
  }
  const normalized = rows.map((row, index) => normalizeRow(row, `rows[${index}]`));
  ...
  let rank = 0;
  if (sigmaMax > EPSILON) rank += 1;
  if (sigmaMinRaw > EPSILON) rank += 1;

  const sigmaMin = rank < 2 ? 0 : sigmaMinRaw;
  const conditionNumber = rank < 2
    ? RANK_DEFICIENT_CONDITION_SENTINEL
    : sigmaMax / sigmaMin;
...
function apertureAuditForGeometry(geometry) {
...
function withCandidateRow(baseRows, row) {
...
export function evaluateEndogenousCandidate(candidate, baseRows = CURRENT_OPERATOR) {
  ...
  const qPre = normalizeRow(candidate.q_pre, `${candidate.candidate_id || 'candidate'}.q_pre`);
  const preGeometry = operatorGeometry(withCandidateRow(baseRows, qPre));
  const preAudit = apertureAuditForGeometry(preGeometry);

  if (candidate.transition_status !== 'DECLARED_SYNTHETIC' || candidate.q_post == null) {
    return deepFreeze({
      candidate_id: String(candidate.candidate_id || 'UNNAMED'),
  ...
  const qPost = normalizeRow(candidate.q_post, `${candidate.candidate_id || 'candidate'}.q_post`);
  const postGeometry = operatorGeometry(withCandidateRow(baseRows, qPost));
  const postAudit = apertureAuditForGeometry(postGeometry);
  const healthy =
    postAudit.deficit_class === 'NO_DECLARED_LOCAL_IDENTIFIABILITY_DEFICIT' &&
    postAudit.disposition === 'ASK_NOTHING';
...
export function naivePreQuestionSelector(evaluations) {
  ...
  const eligible = evaluations
    ...
    .sort((left, right) =>
      left.pre_question.geometry.condition_number - right.pre_question.geometry.condition_number ||
...
export function postQuestionReauditSelector(evaluations) {
  ...
  const eligible = evaluations
    ...
    .sort((left, right) =>
      left.post_question.geometry.condition_number - right.post_question.geometry.condition_number ||
      left.candidate_id.localeCompare(right.candidate_id)
    );
  const selected = eligible[0] || null;
  return deepFreeze({
...
function noDeficitControl() {
  ...
  const aperture = apertureAuditForGeometry(geometry);
  const candidateRankingAllowed = aperture.disposition === 'PROPOSE';
  return deepFreeze({
    geometry,
    aperture,
...
export function runEndogenousObservationReauditGauntlet() {
  ...
  const currentAperture = apertureAuditForGeometry(currentGeometry);
  ...
  const passed =
    ...
    Math.abs(fragile.post_question.geometry.sigma_min - 0.0007071066927903862) < 1e-12 &&
    Math.abs(fragile.post_question.geometry.condition_number - 2000.0005000001252) < 1e-9 &&
  ...
  if (!passed) {
    throw new Error('Endogenous observation re-audit gauntlet violated an authored expectation.');
  }
  ...
  return deepFreeze({
    ...
    bounded_refinement_candidate:
      'candidate-question admissibility may depend on the declared post-question observation/operator state, so pre-question admissibility does not substitute for counterfactual post-question re-audit when intervention dependence is part of the authored model',
    ...
    next_learning_action:
      'TEST_MULTI_STEP_ADAPTIVE_QUESTION_SEQUENCE_WHERE_QUESTION_ORDER_CHANGES_THE_FUTURE_OBSERVATION_OPERATOR_BEFORE_ANY_PATH_DEPENDENT_DESIGN_OR_HOLONOMY_PROMOTION',
    claims: deepFreeze({
