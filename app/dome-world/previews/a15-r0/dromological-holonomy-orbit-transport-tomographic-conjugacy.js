...
function unpackLabelledWords(key) {
  return [
    key & 0xff,
    (key >>> 8) & 0xff,
    (key >>> 16) & 0xff,
...
function isLexSmallerWitness(left, right) {
  ...
  for (let index = 0; index < left.permutation.length; index += 1) {
    if (left.permutation[index] !== right.permutation[index]) {
      return left.permutation[index] < right.permutation[index];
    }
  }
  return false;
}
...
function structuralGeometry() {
  ...
  cachedStructuralGeometry = freezeSmall({
    ...
    affine_cosets_per_flat: [...new Set(cosetCounts)].sort((a, b) => a - b),
  ...
  cachedStructuralGeometry._normalizedFlats = normalizedFlats;
  cachedStructuralGeometry._unlabelledActions = unlabelledActions.action_representatives;
  cachedStructuralGeometry._stabilizers = stabilizers;
...
export function validateDromologicalHolonomyOrbitTransportAtlas(candidate, type) {
  ...
  for (let index = 0; index < candidate.target_keys.length; index += 1) {
    ...
    if (labelledLongType(mapped) !== type) {
      return freezeSmall({ accepted: false, reason: 'H/I/X role not preserved' });
    }
  }
  return freezeSmall({
