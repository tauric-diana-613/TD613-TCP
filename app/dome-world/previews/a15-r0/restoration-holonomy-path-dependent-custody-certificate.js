...
const sortedRecordEntries = record => Object.entries(record).sort(([left], [right]) => left.localeCompare(right));
const sameRecord = (left, right) => canonical(sortedRecordEntries(left)) === canonical(sortedRecordEntries(right));
...
function buildStageProfiles(bundleParent, built, fibres) {
  ...
  function profile(row, stage) {
    ...
    const supports = occupiedKeys.get(`${row.schedule_id}|${stage}`).map(key => {
      ...
      const values = new Set(fibre.antecedents.map(antecedent => canonical(bundleValue(row.claims, antecedent))));
      supportEvaluations += 1;
      return freeze({
        key,
        quotient: fibre.quotient,
        support_keys: freeze([...values].sort()),
        support_cardinality: values.size,
      });
    });
    const value = freeze({
      stage,
  ...
  return { targetsBySchedule, profiles, profile, getSupportEvaluationCount: () => supportEvaluations };
}

function buildPathCensus(bundleParent, built, fibres) {
  const stageProfiles = buildStageProfiles(bundleParent, built, fibres);
    ...
    for (let fine = birth; fine <= 3; fine += 1) {
      for (let intermediate = 1; intermediate < birth; intermediate += 1) {
        ...
        for (let terminal = 0; terminal < intermediate; terminal += 1) {
          pathChecks += 1;
          const dProfile = stageProfiles.profile(row, intermediate);
          const cProfile = stageProfiles.profile(row, terminal);
          ...
          if (transportable) {
            ...
            const terminalMaps = new Map(cProfile.supports.map(support => [
              support.key,
              new Map(support.support_keys.map((valueKey, label) => [valueKey, label])),
            ]));
            for (const target of targets) {
              ...
              const label = terminalMaps.get(terminalKey)?.get(valueKey);
              const intermediateSupport = dProfile.supports.find(support => support.key === intermediateKey);
              const terminalSupport = cProfile.supports.find(support => support.key === terminalKey);
              if (!Number.isInteger(label)
                || label < 0
                || label >= md
                || !intermediateSupport?.support_keys.includes(valueKey)
                || terminalSupport?.support_keys[label] !== valueKey) {
            ...
            legCounts.rupture += 1;
            const maximizingTerminal = cProfile.supports.find(support => support.support_cardinality === mc);
            if (!maximizingTerminal || !(md < maximizingTerminal.support_cardinality)) {
              throw new Error(`missing path lower bound ${row.schedule_id}/${row.bundle_id}/q${fine}->q${intermediate}->q${terminal}`);
            }
            lowerBoundChecks += 1;
          }
  ...
  const endpointTwoPathGroups = [...endpointGroups.values()].filter(group => group.length === 2);
  const mixedGroups = endpointTwoPathGroups.filter(group => (
  ...
  const sameEndpointWitnesses = freeze(mixedGroups.map(group => freeze({
    ...
    paths: freeze([...group]
      .sort((left, right) => left.intermediate - right.intermediate)
      .map(path => freeze({
  ...
  const namedMaximumWitness = paths.find(path => (
  ...
  const supportPairSpectrum = freeze(Object.fromEntries(
    [...pairCounts.entries()].sort(([left], [right]) => left.localeCompare(right)),
  ));
  const scheduleObject = freeze(Object.fromEntries([...bySchedule.entries()].map(([id, counts]) => [id, freeze(counts)])));
  const supportEvaluations = stageProfiles.getSupportEvaluationCount();

  const sameEndpointExact = sameEndpointWitnesses.length === 2
  ...
  const exact = paths.length === 784
    && paths.filter(path => path.birth === 2).length === 160
    && paths.filter(path => path.birth === 3).length === 624
    && plateau === 42
    && rupture === 742
    && strictDecrease === 0
    && sameRecord(supportPairSpectrum, EXPECTED_SUPPORT_PAIRS)
    && supportPairSpectrum['5->5'] === 40
    && supportPairSpectrum['25->25'] === 2
    && canonical(scheduleObject) === canonical(EXPECTED_SCHEDULE)
    ...
    && bySecondLeg['q1->q0'].plateau === 14 && bySecondLeg['q1->q0'].rupture === 354 && bySecondLeg['q1->q0'].total === 368
    && bySecondLeg['q2->q1'].plateau === 24 && bySecondLeg['q2->q1'].rupture === 184 && bySecondLeg['q2->q1'].total === 208
    && bySecondLeg['q2->q0'].plateau === 4 && bySecondLeg['q2->q0'].rupture === 204 && bySecondLeg['q2->q0'].total === 208
    && stageProfiles.profiles.size === 784
    ...
    && endpointTwoPathGroups.length === 208
    && mixedGroups.length === 2
    && sameEndpointExact
    && maximumExpansion === 745
    && namedMaximumWitness?.m_d === 5
    && namedMaximumWitness?.m_c === 750
    && namedMaximumWitness.m_c - namedMaximumWitness.m_d === maximumExpansion
    && localPlateau?.m_d === 5
    && localPlateau?.m_c === 5
    && localPlateau?.transportable === true
    && localRupture?.m_d === 5
    && localRupture?.m_c === 125
    && localRupture?.transportable === false;
  ...
  return freeze({
    ...
    support_pair_spectrum: supportPairSpectrum,
    distinct_support_pair_count: Object.keys(supportPairSpectrum).length,
    plateau_supports: freeze({
      '5->5': supportPairSpectrum['5->5'],
      '25->25': supportPairSpectrum['25->25'],
    }),
    ...
    endpoint_two_path_groups: endpointTwoPathGroups.length,
    ...
    maximum_expansion_witness: namedMaximumWitness ?? null,
    local_cost_nonidentity_witness: freeze({ plateau: localPlateau ?? null, rupture: localRupture ?? null }),
  ...
  cachedCertificate = freeze({
    ...
    transport_criterion: freeze({
      necessary_and_sufficient:
        'PATH_CONDITIONED_LOCALLY_MINIMUM_D_SIDECAR_CAN_REMAIN_EXACT_AFTER_D_IS_DISCARDED_TO_C_IFF_M_D_EQUALS_M_C',
      lower_bound:
        'IF_M_D_IS_SMALLER_THAN_M_C_THE_MAXIMIZING_TERMINAL_FIBRE_HAS_MORE_REQUIRED_BUNDLE_VALUES_THAN_THE_ENTIRE_RETAINED_D_SIDECAR_ALPHABET',
      sufficiency:
        'IF_M_D_EQUALS_M_C_LABEL_REQUIRED_BUNDLE_VALUES_IN_EACH_TERMINAL_FIBRE_AND_RESTRICT_THAT_ASSIGNMENT_TO_NESTED_INTERMEDIATE_FIBRES',
    ...
    scars: freeze([
      ...
      'TIED_MAX_ITERATION_ORDER != WITNESS_AUTHORITY',
    ]),
  });
...
export function rejectRestorationHolonomyOverreach(candidate) {
  const forbidden = [
    ...
    'source_information_creation',
    'retroactive_possession',
    'minimum_bit_length',
    'shannon_capacity',
    'cryptographic_key',
    'authentication_credential',
    'new_sensor_measurement',
    'operational_path_groupoid',
    'physical_holonomy',
    'universal_functoriality',
    'minimum_augmentation_alphabet_from_support_difference',
  ...
  return freeze({
    accepted: !violation,
    reason: violation
      ? 'RESTORATION_HOLONOMY_AUTHORITY_OVERREACH'
      : 'WITHIN_FIXED_FORMAL_RESTORATION_HOLONOMY_MEMBRANE',
  });
}
