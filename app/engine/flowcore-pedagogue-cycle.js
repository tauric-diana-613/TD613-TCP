...
export async function compilePedagogicalScene(input = {}, options = {}) {
  ...
  // Preserve the historical Dome-World ID domain byte-for-byte. Cross-station scenes
  // add ownership to the seed so equivalent content cannot collide across stations.
  if (station_owner !== 'Dome-World') subject.station_owner = station_owner;
  const scene = { schema: PEDAGOGICAL_SCENE_SCHEMA, scene_id: await deterministicId('flowped_scene_', 'TD613:FLOWCORE:PEDAGOGUE-SCENE-ID:v1', subject, options), ...subject, station_owner, anisotropic_legibility: { available_routes: [...FLOWCORE_AIA_ROUTE_IDS], route_selection: 'EXPLICIT_OPERATOR_SELECTION_ONLY', route_registry_schema: FLOWCORE_AIA_ROUTES.schema, route_inference_forbidden: true }, authority: { flowcore_commands_station: false, automatic_ash_action: false, release_authorized: false, station_mutation_authorized: false, human_closure_required: true }, rest: { available: true, penalty: false, continuity_preserved: true, new_demands_withheld: true }, exit: { available: true, penalty: false }, closure: { required: true, status: 'OPEN', closed_by: null } };
