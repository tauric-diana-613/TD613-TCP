...
import { FLOWCORE_AIA_ROUTE_IDS, FLOWCORE_AIA_ROUTES, FLOWCORE_GLYPH_REGISTRY, EVIDENCE_LEVELS, OBSERVATION_STATUSES, PEDAGOGICAL_SCENE_SCHEMA, PEDAGOGICAL_TRANSITION_SCHEMA, PEDAGOGUE_PHASES, SCENE_KINDS, STATIONS, TERMINAL_PHASES, TRANSFER_ENCOUNTER_SCHEMA } from './flowcore-pedagogue-law.js';
...
export async function compilePedagogicalScene(input = {}, options = {}) {
  ...
  const station_owner = String(input.station_owner || 'Dome-World');
  ...
  if (!STATIONS.has(station_owner)) throw new Error(`Unknown Pedagogue scene station: ${station_owner}.`);
  ...
  const subject = { station_owner, scene_kind, source_status, observation_status, world_state_reference: input.world_state_reference ?? null, provenance: provenance(input.provenance), visible_condition: clone(object(input.visible_condition, 'visible_condition')), available_affordances: affordances(input.available_affordances), route_topology: clone(object(input.route_topology, 'route_topology')), causal_structure: clone(object(input.causal_structure, 'causal_structure')), research_frame: research(input.research_frame), missingness: strings(input.missingness || [], 'missingness'), contradictions: strings(input.contradictions || [], 'contradictions'), claim_ceiling: ceiling(input.claim_ceiling), technical_terms_withheld: strings(input.technical_terms_withheld || [], 'technical_terms_withheld') };
  const scene = { schema: PEDAGOGICAL_SCENE_SCHEMA, scene_id: await deterministicId('flowped_scene_', 'TD613:FLOWCORE:PEDAGOGUE-SCENE-ID:v1', subject, options), ...subject, anisotropic_legibility: { available_routes: [...FLOWCORE_AIA_ROUTE_IDS], route_selection: 'EXPLICIT_OPERATOR_SELECTION_ONLY', route_registry_schema: FLOWCORE_AIA_ROUTES.schema, route_inference_forbidden: true }, authority: { flowcore_commands_station: false, automatic_ash_action: false, release_authorized: false, station_mutation_authorized: false, human_closure_required: true }, rest: { available: true, penalty: false, continuity_preserved: true, new_demands_withheld: true }, exit: { available: true, penalty: false }, closure: { required: true, status: 'OPEN', closed_by: null } };
...
export async function compileTransferEncounter(originTransition, newContext = {}, options = {}) {
  ...
  const transfer = { schema: TRANSFER_ENCOUNTER_SCHEMA, transfer_id: await deterministicId('flowped_transfer_', 'TD613:FLOWCORE:PEDAGOGUE-TRANSFER-ID:v1', subject, options), ...subject, system_claim: 'pattern-relation-observed-across-contexts', forbidden_claims: strings(['authorship', 'automatic Ash action', 'cognition', 'identity', 'mastery', 'transfer proves learning', 'transfer proves universal equivalence'], 'forbidden_claims', 1), station_owner: originTransition.station_owner, authority: { automatic_station_advance: false, automatic_ash_action: false, human_closure_required: true }, closure: { status: 'OPEN', closed_by: null } };
