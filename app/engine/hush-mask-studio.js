...
export const HUSH_MASK_STUDIO_VERSION = 'pr188.4-internal-aave-route-hint';
...
function applyInternalRouteHints(mask = {}) {
  if (safeText(mask.id) !== 'phase28-transform-to-aave') return mask;
  return {
    ...mask,
...
export function activeHushMaskIds() { return activeStyleMaskIds(); }
export function listRetiredHushMasks() { return BUILT_IN_MASKS.filter((mask) => retiredStyleMaskReason(mask)).map((mask) => ({ id: mask.id, label: mask.label, retiredReason: retiredStyleMaskReason(mask) })); }
export function listHushMasks(input = {}) { return builtinMasks(input).map((mask) => hydrateHushMask(enrichHushMask(mask))).filter((mask) => input.includeRetiredMasks || mask.active !== false); }
...
export function hydrateHushMask(mask = {}) { const diverseMask = applyStyleDiversity(mask); const protectedMask = applyAuthorshipProtectionToMask(diverseMask); const enriched = applyInternalRouteHints(enrichHushMask(protectedMask)); const sampleSeed = safeText(enriched.sampleSeed); const profile = enriched.profile || extractCadenceProfile(sampleSeed); const distribution = enriched.distribution || buildMaskDistribution(profile); const profileTargets = normalizeProfileTargets(enriched.profileTargets, distribution); const warnings = detectHushMaskWarnings({ ...enriched, profile }); return { version: HUSH_MASK_STUDIO_VERSION, source: 'built-in', ...enriched, sampleSeed, profile, distribution, profileTargets, profileStatus: profileStatus(profile, sampleSeed), profileSummary: summarizeProfile(profile), warnings }; }
export function buildHushMaskProfile(mask = {}, options = {}) { const hydrated = hydrateHushMask(mask); const policy = hydrated.authorshipProtection || buildAuthorshipProtectionPolicy({ mask: hydrated }); return { version: HUSH_MASK_STUDIO_VERSION, maskId: hydrated.id || '', label: hydrated.label || '', active: hydrated.active !== false, retiredReason: hydrated.retiredReason || '', source: hydrated.source || 'built-in', family: hydrated.family || '', description: hydrated.description || '', intendedUse: hydrated.intendedUse || '', riskTell: hydrated.riskTell || '', internalRegister: hydrated.internalRegister || '', routeMetadata: hydrated.routeMetadata || {}, packetHints: hydrated.packetHints || {}, authorshipClass: hydrated.authorshipClass || policy.authorshipClass, syntheticAllowed: Boolean(hydrated.syntheticAllowed || policy.syntheticAllowed), authorshipProtection: policy, profileStatus: hydrated.profileStatus, profile: options.includeFullProfile === false ? {} : hydrated.profile, profileSummary: hydrated.profileSummary, distribution: hydrated.distribution, profileTargets: hydrated.profileTargets, writingTraits: hydrated.writingTraits || {}, transitionBank: asArray(hydrated.transitionBank), dictionHints: asArray(hydrated.dictionHints), avoidList: asArray(hydrated.avoidList), exampleTransformPairs: asArray(hydrated.exampleTransformPairs), diversity: hydrated.diversity || {}, modeAffinity: asArray(hydrated.modeAffinity), intendedContexts: asArray(hydrated.intendedContexts), transformHints: hydrated.transformHints || {}, pressureWarnings: asArray(hydrated.pressureWarnings), warnings: [...asArray(hydrated.warnings), 'built-in-profile-derived-from-seed'], limitations: ['Hush mask profiles are local protective style targets, not external recognition outcomes.', 'Retired near-duplicates stay out of the default studio unless includeRetiredMasks is requested.', 'Protected authorship policy preserves provenance and source safety boundaries.'] }; }
export function summarizeHushMask(mask = {}) { const hydrated = mask.profileSummary ? mask : hydrateHushMask(mask); const policy = hydrated.authorshipProtection || buildAuthorshipProtectionPolicy({ mask: hydrated }); return { id: hydrated.id || hydrated.maskId || '', label: hydrated.label || '', active: hydrated.active !== false, retiredReason: hydrated.retiredReason || '', family: hydrated.family || '', authorshipClass: hydrated.authorshipClass || policy.authorshipClass, syntheticAllowed: Boolean(hydrated.syntheticAllowed || policy.syntheticAllowed), profileStatus: hydrated.profileStatus || 'empty', profileSummary: hydrated.profileSummary || summarizeProfile(hydrated.profile || {}), distribution: hydrated.distribution || buildMaskDistribution(hydrated.profile || {}), profileTargets: hydrated.profileTargets || normalizeProfileTargets({}, hydrated.distribution || buildMaskDistribution(hydrated.profile || {})), writingTraits: hydrated.writingTraits || {}, diversity: hydrated.diversity || {}, warnings: asArray(hydrated.warnings), riskTell: hydrated.riskTell || '' }; }
