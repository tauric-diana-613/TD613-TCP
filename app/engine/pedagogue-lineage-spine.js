...
export const PEDAGOGUE_LINEAGE_SPINE_SCHEMA = 'td613.flowcore.pedagogue-lineage-spine/v0.1';
...
export const PEDAGOGUE_LINEAGE_NODES = Object.freeze([
  freeze({
    ...
    kind: 'THINKER',
    introduced_in: 'ASP_EARLY',
    routed_through: freeze(['DOME_WORLD_CHILD_LIBERATION', 'TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze([
  ...
  freeze({
    ...
    kind: 'THINKER',
    introduced_in: 'DOME_WORLD_CHILD_LIBERATION',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['Experience and Education (1938)']),
    relations: freeze([
  ...
  freeze({
    ...
    introduced_in: 'ASP_EARLY',
    routed_through: freeze(['DOME_WORLD_CHILD_LIBERATION', 'TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['For Your Own Good']),
    relations: freeze([
  ...
  freeze({
    ...
    introduced_in: 'ASP_EARLY',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['ASP advocacy lineage as supplied by Potato; exact source work unresolved in current corpus']),
    relations: freeze([
  ...
  freeze({
    ...
    kind: 'THINKER',
    introduced_in: 'ASP_EARLY',
    routed_through: freeze(['FLOW_CORE_GOVERNANCE', 'DOME_WORLD_CHILD_LIBERATION', 'TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['Debt: The First 5,000 Years', 'The Utopia of Rules', 'Dead Zones of the Imagination']),
    relations: freeze([
  ...
  freeze({
    ...
    kind: 'THINKER_PAIR',
    introduced_in: 'FLOW_CORE_GOVERNANCE',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['The Dawn of Everything (2021)']),
    relations: freeze([
  ...
  freeze({
    ...
    introduced_in: 'DOME_WORLD_CHILD_LIBERATION',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['Dumbing Us Down (1992)']),
    relations: freeze([
  ...
  freeze({
    ...
    introduced_in: 'DOME_WORLD_CHILD_LIBERATION',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['The Death and Life of Great American Cities (1961)']),
    relations: freeze([
  ...
  freeze({
    ...
    introduced_in: 'DOME_WORLD_CHILD_LIBERATION',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['Potato Phase 3 routed architectural discussion; living knowledge not directly represented by TD613']),
    relations: freeze([
  ...
  freeze({
    ...
    introduced_in: 'FLOW_CORE_GOVERNANCE',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['Potato Phase 3: engaged through reflective study rather than direct representation']),
    relations: freeze([
  ...
  freeze({
    ...
    introduced_in: 'FLOW_CORE_GOVERNANCE',
    routed_through: freeze(['TD613_PEDAGOGUE_OPERATIONALIZATION']),
    sources: freeze(['Potato Phase 3: process-oriented understandings of balance and flow in traditional Chinese medical thought']),
    relations: freeze([
...
export function tracePedagogueLineage(nodeId) {
  ...
  return freeze({
    schema: 'td613.flowcore.pedagogue-lineage-trace/v0.1',
    node,
  ...
  const lenses = unique.map((id) => {
    ...
    return freeze({
      ...
      provenance: freeze(lens.provenance_nodes.map((nodeId) => {
        ...
        return freeze({ node_id: node.node_id, display_name: node.display_name, introduced_in: node.introduced_in, routed_through: node.routed_through });
      }))
    });
  });
