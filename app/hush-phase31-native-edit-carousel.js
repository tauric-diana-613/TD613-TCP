const VERSION = 'phase-31.1-native-edit-carousel/v2';
...
const DISCOURSE_MODES = Object.freeze(['explanatory','argumentative','narrative','procedural','reflective-affective','legal-forensic','casual-conversational','technical-operational','poetic-symbolic','corrective-repair','compressed-summary']);
const RETRIEVAL_TRIGGERS = Object.freeze(['baseline-voice','high-pressure','failure-recovery','correction-request','disagreement-pushback','implementation-handoff','evidence-framing','boundary-refusal','uncertainty-caveat','deep-explanation','compression-summary','affective-repair','ritual-symbolic','public-facing','private-diagnostic']);
const DISCOURSE_ALIASES = Object.freeze({ reflective: 'reflective-affective', casual: 'casual-conversational', technical: 'technical-operational', repair: 'corrective-repair', 'legal / forensic': 'legal-forensic', 'casual / conversational': 'casual-conversational', 'technical / operational': 'technical-operational', 'poetic / symbolic': 'poetic-symbolic', 'repair / correction': 'corrective-repair' });
...
function normalizeSampleEntry(entry) {
  ...
  if (typeof entry === 'string') {
    return { text: body, promptCategory: DISCOURSE_MODES[0], contextLabel: RETRIEVAL_TRIGGERS[0], discourseMode: DISCOURSE_MODES[0], retrievalTrigger: RETRIEVAL_TRIGGERS[0] };
  }
  const discourseMode = optionValue(entry?.discourseMode || entry?.promptCategory || entry?.contextLabel, DISCOURSE_MODES, DISCOURSE_ALIASES);
  const retrievalTrigger = optionValue(entry?.retrievalTrigger || entry?.contextLabel, RETRIEVAL_TRIGGERS);
  return { text: body, promptCategory: discourseMode, contextLabel: retrievalTrigger, discourseMode, retrievalTrigger };
}
...
export function writeStoredSamples(nextSamples = []) {
  ...
  try {
    ...
    else localStorage.setItem(STORAGE_KEY, JSON.stringify({ version: 'phase31-logged-samples/v1', updatedAt: new Date().toISOString(), samples: clean }));
  } catch (error) {}
  return clean;
}

function corpusSignature(nextSamples = []) {
  return JSON.stringify(asArray(nextSamples).map(normalizeSampleEntry).filter(Boolean).map((sample) => ({ text: sample.text, promptCategory: sample.promptCategory, contextLabel: sample.contextLabel, discourseMode: sample.discourseMode, retrievalTrigger: sample.retrievalTrigger })));
}
    ...
    : generationAllowed
      ...
      : acceptedSampleCount >= PROVISIONAL_SAMPLES && acceptedWords >= PROVISIONAL_WORDS
        ...
        : clean.length
          ? 'corpus-building'
          : 'empty';
  return { status, acceptedSampleCount, acceptedWords, promptCategoryCount, readinessScore: Math.min(1, Math.max(acceptedSampleCount / RIGOROUS_SAMPLES, acceptedWords / RIGOROUS_WORDS)), generationAllowed };
}

function createMask(nextSamples = []) {
  const clean = asArray(nextSamples).map(normalizeSampleEntry).filter(Boolean);
  const readiness = readinessFor(clean);
  return clean.length ? { version: 'phase31-light-custom-mask/v1', id: 'custom-unsaved-phase31-1', label: 'Unsaved Custom Mask', name: 'Unsaved Custom Mask', source: 'custom-unsaved-phase31-light', samples: clean, sampleCount: clean.length, acceptedSampleCount: readiness.acceptedSampleCount, acceptedWords: readiness.acceptedWords, profileStatus: readiness.status, corpusReadiness: readiness, sampleSeed: clean.map((sample) => sample.text).join('\n\n') } : null;
}
...
function scrubOldEditListeners(doc = document) {
  [
    ['hushPhase31EditCorpus', ['td613Phase31NativeEditCorpusBound']],
    ['hushPhase31CloseCorpusEdit', ['td613Phase31NativeCloseCorpusEditBound']],
    ['hushPhase31SaveCorpusEdits', ['td613Phase31NativeSaveCorpusEditBound']],
    ['hushPhase31EditCorpusList', ['td613Phase31NativeRemoveCorpusEditBound']]
  ].forEach(([id, legacyKeys]) => {
    const node = byId(id, doc);
    if (!node) return;
    legacyKeys.forEach((key) => { if (node.dataset) node.dataset[key] = 'true'; });
    if (node.dataset?.td613Phase31NativeCarouselScrubbed === 'true') return;
    const clone = node.cloneNode(true);
    if (clone.dataset) {
      clone.dataset.td613Phase31NativeCarouselScrubbed = 'true';
      legacyKeys.forEach((key) => { clone.dataset[key] = 'true'; });
    }
...
export function pullActiveEditCorpusSample(doc = document) {
  ...
  const updated = normalizeSampleEntry({ text: row.querySelector('.hush-phase31-edit-text')?.value || '', promptCategory: discourseMode, contextLabel: retrievalTrigger, discourseMode, retrievalTrigger });
...
function removeActiveEditCorpusSample(doc = document) {
...
export function installNativeCorpusCarousel(doc = document) {
  ensureCarouselChrome(doc);
  scrubOldEditListeners(doc);
  bindOnce(doc.documentElement, 'click', 'td613Phase31NativeCarouselRootCaptureBound', (event) => {
    if (event.target?.closest?.('#hushPhase31EditCorpus')) return intercept(event, () => openEditCorpusModal(doc));
    if (event.target?.closest?.('#hushPhase31PrevSample')) return intercept(event, () => moveEditCorpus(-1, doc));
    if (event.target?.closest?.('#hushPhase31NextSample')) return intercept(event, () => moveEditCorpus(1, doc));
    if (event.target?.closest?.('.hush-phase31-edit-remove')) return intercept(event, () => removeActiveEditCorpusSample(doc));
    if (event.target?.closest?.('#hushPhase31CloseCorpusEdit')) return intercept(event, () => closeEditCorpusModal(doc));
    if (event.target?.closest?.('#hushPhase31SaveCorpusEdits')) return intercept(event, () => saveEditCorpusModal(doc));
  }, true);
  ...
  bindOnce(byId('hushPhase31EditCorpusList', doc), 'click', 'td613Phase31NativeCarouselRemoveCorpusEditBound', (event) => intercept(event, () => removeActiveEditCorpusSample(doc)), true);
}

export function exposeNativeCorpusCarousel(doc = document) {
  window.__TD613_HUSH_PHASE31_NATIVE_EDIT_CAROUSEL__ = { version: VERSION, openEditCorpusModal, closeEditCorpusModal, renderActiveEditCorpusSample, moveEditCorpus, pullActiveEditCorpusSample, saveEditCorpusModal, readStoredSamples, writeStoredSamples, installNativeCorpusCarousel };
