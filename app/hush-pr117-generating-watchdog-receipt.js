(function(){'use strict';
var V='pr117-diagnostic-receipt-popup/v3-compact-full-copy';
...
function normalize(data){
  ...
  return {
    status:(data&&data.status)||(payload&&payload.status)||(payload&&payload.ok===false?'held':'diagnostic'),
    reason:(data&&data.reason)||(payload&&payload.reason)||(payload&&payload.error)||(payload&&payload.message)||(meta&&meta.reason)||'provider diagnostic receipt',
    endpoint:(data&&data.endpoint)||(payload&&payload.endpoint)||(meta&&meta.endpoint)||'unknown',
    httpStatus:(data&&data.httpStatus)||(payload&&payload.httpStatus)||(meta&&meta.httpStatus)||'n/a',
    fallbackReleased:data&&data.fallbackReleased===true?'yes':'no',
    bridgeVersion:(data&&data.bridgeVersion)||(payload&&payload.bridgeVersion)||(meta&&meta.bridgeVersion)||'',
    raw:data
  };
}
function compactReceipt(normalized){
  var raw=normalized.raw||{};
  var payload=raw.payload||{};
  return {
    schema:'td613-hush-diagnostic-receipt/compact-v1',
    status:normalized.status,
    reason:normalized.reason,
    endpoint:normalized.endpoint,
    httpStatus:normalized.httpStatus,
    fallbackReleased:normalized.fallbackReleased,
    bridgeVersion:normalized.bridgeVersion||raw.bridgeVersion||'',
    payloadError:payload.error||payload.reason||payload.message||'',
    promptVersion:raw.contract&&raw.contract.promptVersion||'',
    packetVersion:raw.contract&&raw.contract.flightPacket&&raw.contract.flightPacket.packet_version||'',
    maskId:raw.contract&&raw.contract.maskId||raw.contract&&raw.contract.flightPacket&&raw.contract.flightPacket.mask_id||'',
    sourceWordCount:raw.contract&&raw.contract.sourceText?String(raw.contract.sourceText).match(/[A-Za-z0-9][A-Za-z0-9'-]*/g)?.length||0:0
  };
}
function copyText(value,button,success){
  try{navigator.clipboard.writeText(value);button.textContent=success||'Copied';}
...
function boot(){
  ...
  installStyle();
  window.TD613_HUSH_PR117={version:V,disabled:false,receipt:receipt,force:force,arm:function(){return receipt()},pulse:function(){return receipt()},compactReceipt:compactReceipt};
