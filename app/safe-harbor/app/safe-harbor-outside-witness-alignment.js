...
function statusForWitness(value, requiredKeys = []) {
  if (!value) return 'unavailable';
  if (value.status === 'refused' || value.recommended_action === 'refuse' || value.recommended_action === 'block' || value.can_countersign === false) return 'refused';
  for (const key of requiredKeys) if (!getPath(value, key)) return 'pending';
  return 'aligned';
}
