import {
  ...
  buildRendererAuthorityMetadata,
...
function listChangedPaths(left, right, prefix = '') {
  ...
  return keys.flatMap((key) => listChangedPaths(left[key], right[key], prefix ? `${prefix}.${key}` : key));
}

function hasIssuedV3(packet) {
  const v3 = packet && packet.issuance && packet.issuance.v3;
  return Boolean(v3 && v3.status === 'issued' && v3.badge_number_v3 && v3.stylometric_fingerprint_v3);
}
...
function publicDefaultConflict(packet) {
  ...
  if (policy.v3_public_ready === true && promotion !== 'v3-public-default-ready') {
    return { path: 'public_default_policy.v3_public_ready', expected: false, actual: true, severity: 'high', recommended_action: 'demote-public-default' };
  }
  if (policy.default_public_credential !== 'v2' && promotion !== 'v3-public-default-ready') {
    return { path: 'public_default_policy.default_public_credential', expected: 'v2', actual: policy.default_public_credential, severity: 'high', recommended_action: 'demote-public-default' };
  }
  return null;
}
...
export async function detectStaleV3(packet) {
  ...
  if (!stored || stored.status !== 'issued') {
    return Object.freeze({ status: 'not-issued', action: 'none', rebuild_allowed: false, silent_rebuild_allowed: false });
  }
  const replay = await verifyV3Replay(packet);
  if (replay.status === 'pass') {
    return Object.freeze({ status: 'fresh', action: 'none', rebuild_allowed: false, silent_rebuild_allowed: false, replay_status: replay.status });
  }
...
export async function buildConvergenceReport(normalizeFn, packet, options = {}) {
  if (typeof normalizeFn !== 'function') {
    return Object.freeze({ status: 'unavailable', iterations: 0, stable_after_iteration: null, changed_paths: [], volatile_paths_excluded: VOLATILE_PATHS.slice() });
  }
  ...
  const changed_paths = snapshots.length >= 2 && snapshots[snapshots.length - 1] !== snapshots[snapshots.length - 2]
    ...
    : [];
  return Object.freeze({
    status: stableAfter ? 'pass' : 'fail',
    iterations,
    stable_after_iteration: stableAfter,
    changed_paths,
    volatile_paths_excluded: VOLATILE_PATHS.slice()
  });
}
...
export async function buildHashReplayBattery(packet) {
  ...
  return Object.freeze({
    ...
    detail: {
      renderer_mismatches: renderer.mismatches,
      phase4_intake_mismatches: intake.mismatches,
      authority
    }
  });
}
...
export async function buildAuthorityConflictReport(packet) {
  ...
  if (renderer.status !== 'unavailable' && rendererMetadataReport(packet).mismatches.some((item) => item.path === 'renderer_authority_metadata.badge_number_v3') && !v3.badge_number_v3) conflicts.push({ path: 'renderer_authority_metadata.badge_number_v3', expected: null, actual: getPath(packet, 'renderer_authority_metadata.badge_number_v3'), severity: 'high', recommended_action: 'quarantine-public-artifact' });
...
export async function buildTamperReport(packet) {
  const fixtures = [
    'tamper_v2_badge',
    'tamper_v2_fingerprint',
    'tamper_v3_badge',
    'tamper_v3_fingerprint',
    'tamper_lane_rich_profile_scalar',
    'tamper_lane_distribution',
    'delete_hash_semantics',
    'delete_packet_authority_surface',
    'promote_public_default_without_authority',
    'renderer_hash_mismatch',
    'phase4_intake_dual_verify_without_v3',
    'bridge_only_fake_v3_authority'
  ];
  const results = [];
  for (const fixture of fixtures) {
    ...
    const actual_status = detected_by.length ? 'fail' : 'pass';
    results.push({ fixture, expected_status: 'fail', actual_status, detected_by });
  }
  return Object.freeze({
    status: results.every((item) => item.actual_status === item.expected_status) ? 'pass' : 'fail',
    fixtures: results
  });
}
...
export async function buildChallengeReplayReport(packet, challengeProfile) {
  ...
  const rebuiltHash = challengeProfile.fresh_profile_hash ? 'pass' : 'unavailable';
  ...
  return Object.freeze({
    status: lane_failures.length ? 'fail' : 'pass',
    raw_text_retained: challengeProfile.raw_text_retained === false,
    profile_hash_replay: rebuiltHash,
    continuity_score_v3: comparison.continuity_score_v3,
    continuity_band: comparison.continuity_band,
    minimum_lane_count: 3,
    lane_failures,
    recommended_action: recommended,
    receipt: buildRecallChallengeReceipt(packet, challengeProfile)
  });
}
...
export async function buildStep1AuthorityReview(packet) {
  ...
  const publicDefaultStatus = hardening.authority_conflict_report.conflicts.some((item) => String(item.path || '').indexOf('public_default_policy') === 0)
    ? 'conflict'
    : publicPolicy.default_public_credential || 'v2';
  let recommended = hardening.recommended_action;
  if (hardening.status === 'pass' && hardening.hash_replay_battery.v3_badge === 'pass') recommended = 'dual-verify';
  return Object.freeze({
    phase4_recall_intake_present: Boolean(packet && packet.phase4_recall_intake),
    phase5_replay_hardening_present: Boolean(packet && packet.phase5_replay_hardening),
    recommended_action: recommended,
    v3_replay_status: hardening.hash_replay_battery.v3_badge,
    hash_replay_status: hardening.hash_replay_battery.packet_hash,
    public_default_status: publicDefaultStatus,
    operator_warning: hardening.status === 'pass' ? null : 'Phase 5 replay hardening did not pass; do not countersign as clean.'
  });
}
...
export async function shouldAllowPublicExport(packet) {
  const hardening = packet && packet.phase5_replay_hardening ? packet.phase5_replay_hardening : await buildPhase5ReplayHardening(packet, { includeTamperFixtures: false });
  return Object.freeze({
    allowed: hardening.status === 'pass' || hardening.status === 'review',
    recommended_action: hardening.recommended_action,
    reason: hardening.status === 'pass' ? 'Phase 5 replay hardening passed.' : 'Public export requires review or quarantine due to replay hardening status.'
  });
}
...
export async function buildPhase5ReplayHardening(packet, options = {}) {
  ...
  const tamper_failures = tamper_report.status === 'fail';
  let status = 'pass';
  let recommended_action = 'export';
  if (stale_v3_policy.status === 'stale-detected') { status = 'quarantine'; recommended_action = 'quarantine'; }
  else if (authority_conflict_report.status === 'conflict') { status = 'quarantine'; recommended_action = 'quarantine'; }
  else if (replay_battery.packet_hash === 'fail' || replay_battery.v2_badge === 'fail' || replay_battery.v3_badge === 'fail' || replay_battery.renderer_metadata === 'fail' || replay_battery.phase4_intake === 'fail') { status = 'fail'; recommended_action = 'block'; }
  else if (tamper_failures || convergence_report.status === 'fail') { status = 'fail'; recommended_action = 'block'; }
...
export function applyPhase5Quarantine(packet, hardening) {
  ...
  if (hardening && (hardening.status === 'quarantine' || hardening.status === 'fail')) {
    out.export_quarantine = {
      schema_version: 'td613.safe-harbor.export-quarantine/v1',
      status: hardening.status,
      recommended_action: hardening.recommended_action,
      reason: hardening.status === 'quarantine' ? 'Phase 5 replay hardening detected stale or contradictory authority.' : 'Phase 5 replay hardening failed adversarial replay.'
    };
  }
  return out;
}
...
if (typeof window !== 'undefined') {
  window.TD613_SAFE_HARBOR_PHASE5 = Object.freeze({
    ...
    buildStep1AuthorityReview,
    shouldAllowPublicExport,
    applyPhase5Quarantine
  });
