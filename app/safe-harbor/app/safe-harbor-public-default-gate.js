...
export function buildPhase8RendererPolicy(packet, gate) {
  ...
  const existing = getPath(packet, 'renderer_authority_metadata') || {};
  return Object.freeze({ ...existing, schema_version: RENDERER_SCHEMA, badge_number: getPath(packet, 'issuance.badge_number') || null, badge_number_v3: getPath(packet, 'issuance.v3.badge_number_v3') || null, v3_status: existing.v3_status || getPath(packet, 'issuance.v3.status') || 'unavailable', promotion_status: existing.promotion_status || getPath(packet, 'recall_governance.v3.promotion_status') || 'v3-not-yet-recall-authoritative', v2_role: roles.v2, v3_role: roles.v3, public_default_credential: 'v2', public_display_mode: gate ? gate.public_default_after : 'v2-only', phase8_gate_status: gate ? gate.status : 'unavailable', phase8_gate_decision: gate ? gate.gate_decision : 'keep-v2-only', native_spine_status: nativeSpineStatus(packet), phase5_status: phase5Status(packet), outside_witness_alignment: outsideStatus(packet), packet_hash_sha256: packetHash(packet), hash_topology_final: hashTopologyFinal(packet), raw_text_included: false, renderer_claim_limit: 'Renderer metadata displays Phase 8 policy; it does not create public authority.' });
}
