# TD613 Hush Operator Guide

TD613 Hush is a quiet toy inside The Cadence Playground. It is playful on the surface and disciplined under the hood. For most visitors it reads as another cadence experiment. For whistleblowers and other vulnerable operators, it can become a local review route for authorship-recognition pressure.

## Before using Hush

Use Hush only as a local review instrument. It helps inspect source residual pressure, mask fit, semantic preservation, protected literals, ingestion friction, Persona memory, claim ceiling, and Recognition Field context pressure.

Local seal means bounded local convergence. It does not mean safe to publish.

## The basic workflow

1. Stage the protected baseline.
2. Stage a mask or Persona.
3. Stage the message draft.
4. Generate or edit the protected output.
5. Analyze Escape Vector.
6. Review Ingestion Friction.
7. Review Controller state.
8. Review Persona Memory.
9. Review Claim Ladder.
10. Review Recognition Field.
11. Export a report only when the claim ceiling supports it.

## Choosing a Persona

Personas remain social objects, not sterile risk profiles. They can be avatars, group-chat characters, and practice surfaces. Hush measures them as exposure membranes because repeated Persona use can create local continuity, drift, or overfit.

Choose a Persona based on the communication field, not vibes alone. A warm group-chat Persona and a document-handoff Persona have different pressure profiles.

## Stable pseudonym vs rotating mask

Stable pseudonym mode can support repeated local continuity when the goal is a consistent mask surface. Rotating mask mode treats repeated continuity as pressure that may call for rotation or cooldown.

Persona continuity is strategic only when it matches operator intent.

## Hostile Pipeline Compression

Hostile Pipeline Compression is a lossy situational mode for bureaucratic ingestion surfaces such as legal intake, HR portals, compliance forms, and document handoff notes. It can preserve facts, dates, exhibits, and custody markers while reducing expressive heat.

It is not universal safety. Smoothness is not protection by itself.

## Belonging Without Collapse

Belonging Without Collapse means readability inside the intended field without surrendering the protected speaker's evidence, meaning, or boundary. In group chat, this may mean warmth and ordinary phrasing. In document handoff, this may mean concise custody language.

Context legibility is not obedience to the host field. It is readability without authorship collapse.

## Protected literals

Protected literals include exhibit IDs, dates, names, reference labels, statutes, code strings, glyphs, and ZWNJ boundaries. Hush treats them as evidence pressure. A protected literal can be necessary and exposing at the same time.

When a protected literal drops, restore before celebrating a style score.

## When to hold

Hold when the controller says evidence is thin, semantic fidelity is weak, source residual remains high, Recognition Field route is hot, or the Claim Ladder limits the conclusion.

A hold state is not failure. It is the instrument refusing to flatter you.

## When to restore

Restore when meaning, dates, exhibits, protected literals, or witness context have been lost. Restoration should outrank mask fit.

## When to rotate

Rotate when Persona continuity, mask overuse, Recognition Field pressure, or repeated phrasing makes the current Persona too indexable for the intended mode.

## When to seal locally

Seal locally only when the output preserves meaning, keeps required evidence, remains within claim limits, and survives context review.

Local seal means bounded local convergence. It does not mean safe to publish.

## How to export reports

Use report export after analysis. Default reports exclude private text and include metrics, summaries, limitations, claim ceilings, and Recognition Field information when supplied.

Do not paste private baseline, draft, mask reference, or output into reports unless you intentionally opt into that exposure in a controlled environment.

## What reports can claim

Reports can describe local authorship-recognition pressure, local mask-fit movement, local reduced source-linkage candidate status, semantic preservation status, protected literal status, ingestion friction, Persona memory posture, and context-pressure warnings.

## What reports cannot claim

Reports cannot prove identity, anonymity, untraceability, platform outcome, publication safety, or external classifier behavior.

The Claim Ladder is the ceiling on permitted language, not a confidence trophy.

## Whistleblower-specific caution

When evidence, employment, legal exposure, immigration status, health, housing, custody, or retaliation risk is involved, Hush output should be treated as a review aid, not a legal strategy substitute.

A whistleblower can use Hush to review whether a draft preserves evidence while reducing certain local authorship-recognition pressures. Hush cannot determine whether an employer, agency, platform, vendor, or adversary will identify the speaker.

## Emergency rule: do not publish because the tool sounded confident

Hush is designed to resist false certainty, but operators must still refuse overconfidence. Do not publish, submit, leak, or send sensitive material solely because a local score looked favorable.

When the stakes are real, get human review, legal guidance, or trusted safety review before exposure.
