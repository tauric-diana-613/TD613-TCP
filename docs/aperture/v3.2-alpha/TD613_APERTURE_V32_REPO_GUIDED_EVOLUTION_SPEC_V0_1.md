...
This specification governs the standalone v3.2-alpha candidate authored from the canonical v3.1-alpha standalone. It does **not** install v3.2 repo-wide, update `app/aperture/release.json`, promote the repository identity, deploy TD613.com, or authorize Vercel.

## 0. Why v3.2 exists now

v3.1 already installed a distributed observatory architecture:
...
The phase-free A15-R0 research frontier on Draft PR #683 has since added bounded synthetic work on:
...
The regression risk is therefore not that v3.1 lacks another reconstruction engine. The risk is that the standalone instrument still knows how to report reconstruction/abstention but does not compactly express **what kind of insufficiency exists before widening or whether another question is warranted at all.**

v3.2 closes that gap without moving the full research laboratory out of Dome-World.

## 1. Governing objective

v3.2 adds a compact **Typed Epistemic Deficit + Question-Need Audit** to standalone Aperture.
...
The repository houses both root shortcuts:
...
The richer Pedagogue research metabolism currently lives on Draft PR #683 / `research/a15-r0-identifiability-frontier`; this clean ingress branch does not silently install that entire research body.
...
The current research frontier materially supports methodological inheritance such as:
...
Do not globally relabel historical contracts as v3.2.

One final v3.2 identity synchronizer may harden the standalone against inherited version writers, but it must be idempotent and must not create an uncontrolled animation or mutation-observer loop.

## 9. Patch Bay / agent routing

A v3.2 agent encountering one of the following should consider the new audit:
...
The standalone v3.2 candidate returns through the existing bidirectional lane:
...
Prefer an additive v3.2 compatibility/deficit-audit layer over renaming v3.1 engine files unless a specific v3.1 defect requires migration.
...
before proposing,
establish that another question is needed;
