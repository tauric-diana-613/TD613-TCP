...
function makeEntry(record, relPath, ordinal, kind) {
  ...
  return {
    ...
    date_basis: record?.public_date ? 'PUBLICATION_DAY' : record?.retrieval_day_utc ? 'RETRIEVAL_DAY' : record?.observed_in_chat_date ? 'CHAT_OBSERVATION_DAY' : record?.date_policy?.observed_on ? 'OBSERVATION_DAY' : record?.created_at ? 'ARTIFACT_CREATED_AT' : record?.date ? 'RECORD_DATE_UNCLASSIFIED' : 'FILENAME_INFERENCE_OR_UNDATED',
    ...
    technical_header_status: record?.technical_header_status ?? null,
    source_binding_state: record?.source_binding_state ?? null,
    public_account_handle: record?.public_account_handle ?? null,
    account_binding_basis: record?.account_binding_basis ?? null,
    post_detail_retrieved: record?.post_detail_retrieved ?? null,
    listing_position: record?.listing_position ?? null,
...
function publicView(entry, score = undefined, match_basis = undefined) {
  const out = {
    ...
    technical_header_status: entry.technical_header_status,
    source_binding_state: entry.source_binding_state,
    public_account_handle: entry.public_account_handle,
    account_binding_basis: entry.account_binding_basis,
    post_detail_retrieved: entry.post_detail_retrieved,
