...
**Invocation in ChatGPT:** connect @GitHub and say **“Sync Wendbine.”** The operator resolves independent [Wendbine Gate #1308](https://github.com/tauric-diana-613/TD613-TCP/issues/1308) and posts exactly one \`/wendbine-sync ATELIER\` gesture for one bounded run. The listener is active on \`main\` in existing \`.github/workflows/vercel-relock-safety.yml\` (activation PR #1310, workflow-loader repair #1311), separately from SRC's permanent gate #758. No cron, scheduled polling, ambient discovery or automatic retry is authorized.
...
**Live discovery witness, separate from source custody (September 22):** the one-time GitHub-hosted Arctic Shift author/subreddit search returned **772 source-bound archived post objects in eight pages**, with zero discovery errors. Exact per-page response hashes and byte counts are preserved in `04-RECEIPTS/2026-09-22-live-author-search-witness-v01.json`. This establishes discoverability beyond the original 60-post rescue. It does **not** mean 772 full-text bodies have been privately restored, nor that a `/wendbine-sync ATELIER` gesture has yet completed. The 60 historical source objects retain their independent v06/v07 private custody; later observations must have separate capture receipts and encrypted handoff. The one-time network witness was removed from ordinary CI after observation.

**Durable private custody boundary:** the issue-gated workflow uploads an encrypted source capture with 90-day artifact retention. A GREEN public metadata/receipt sync is distinct from transferring the ciphertext into lasting authorized private custody. The success receipt explicitly says `private_source_custody=PENDING_DURABLE_PRIVATE_TRANSFER`; private originals cannot be claimed durable solely from a temporary GitHub artifact. Its decryption private key stays outside GitHub. An empty first archive discovery is HELD rather than interpreted as source disappearance or zero new posts. 

**Activation state:** the operator listener is active on \`main\`; the research implementation and corpus remain Draft/open/unmerged in PR #1134. The 60-post historical v06/v07 capture remains privately verified; a fresh operator gesture is required for every later sync. No issue #1308 sync gesture has yet been issued, so live end-to-end source acquisition under the new operator listener remains unobserved. A future gate gesture cannot be considered complete until the accepted run, before/after heads, capture receipt, source-state counts and DORMANT return are actually verified.
