...
async function waitVisibleWorkspace(page,name){await page.waitForFunction(workspace=>{const panel=document.getElementById(`workspace-${workspace}`);const style=panel?getComputedStyle(panel):null;const rect=panel?.getBoundingClientRect();return document.documentElement.dataset.ashPremiumWorkspace===workspace&&panel?.classList.contains('active')&&style?.display!=='none'&&style?.visibility!=='hidden'&&Number(style?.opacity)>0&&style?.pointerEvents!=='none'&&rect?.width>0&&rect?.height>0;},name);}
...
try{
  ...
  await page.waitForFunction(version=>Boolean(window.__td613AshGuidedOperatorUI?.version)&&Boolean(window.__td613AshFlickerHardening?.version)&&Boolean(window.__td613AshDemoEntryConvergence?.version)&&window.__td613AshInvestigationDemo?.version===version,VERSION);
  ...
  await page.waitForFunction(()=>document.documentElement.dataset.ashDemoEntryReady==='investigation:home'&&document.documentElement.dataset.ashDemoEntryHydrating!=='true'&&!document.documentElement.dataset.ashDemoEntryHold&&window.__td613AshDemoEntryConvergence?.current?.().posture==='READY');
  ...
  await waitVisibleWorkspace(page,'map');
  const controlLabels=(await page.locator('#guidedMapControlLegend button').allTextContents()).map(value=>value.trim());
