...
async function runProfile(browser, profile, options, report) {
  ...
  requireValue(JSON.stringify(governedLifecycle(after.lifecycle)) === JSON.stringify(governedLifecycle(before.lifecycle)), `${profile}: lesson changed governed lifecycle semantics.`);
...
const report = {
  schema: 'td613.ash.aia2-task-continuity-browser-evidence/v0.4',
