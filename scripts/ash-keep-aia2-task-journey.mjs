...
const artifactDir = path.resolve(process.env.TD613_ARTIFACT_DIR || `artifacts/ash-aia2-${browserName}`);
...
async function snapshot(page, label) {
  return page.evaluate(labelValue => {
    ...
    const hasHorizontalScrollLane = node => {
    ...
    const controls = [...document.querySelectorAll(
      '#ashAiaMembrane button, #ashAiaMembrane input, #ashAiaMembrane select, body > main button, body > main input, body > main select, body > .workspace-rail button'
    )].filter(visible);
    const clipped = controls.filter(node => {
      const rect = node.getBoundingClientRect();
      return !hasHorizontalScrollLane(node) && (rect.left < -1 || rect.right > innerWidth + 1);
...
async function firstUse(page, profile, report) {
  const initial = await snapshot(page, `${profile}-first-use`);
  assert(initial.root_visible, `${profile}: task guide is not visible.`);
  assert(initial.active_route === 'EXPERIENTIAL', `${profile}: default route is ${initial.active_route}.`);
  assert(initial.task === 'setup', `${profile}: first task is ${initial.task}.`);
  assert(initial.ingress_visible && initial.profile_visible && initial.new_case_visible && initial.demo_visible,
    `${profile}: exact ingress controls are not visible.`);
  assert(initial.ingress_position !== 'fixed', `${profile}: ingress still blocks the viewport as a fixed overlay.`);
  assert(initial.new_case_in_viewport, `${profile}: New Case is outside the initial viewport.`);
  assert(initial.document.overflow <= 2, `${profile}: horizontal overflow ${initial.document.overflow}px.`);
  assert(initial.clipped_controls.length === 0, `${profile}: clipped controls: ${initial.clipped_controls.join(', ')}.`);
  assert(initial.duplicate_ids.length === 0, `${profile}: duplicate IDs: ${initial.duplicate_ids.join(', ')}.`);
  assert(initial.body_held !== 'true', `${profile}: presentation held.`);
...
async function openLocalDocument(page, profile, report) {
  ...
  assert(before.active_workspace === 'workspace-draft' && before.local_file_visible,
    `${profile}: local file control is unavailable.`);
  const text = `TD613 AIA2 synthetic local document for ${profile}; no recipient route.`;
  await page.locator('#localTextFile').setInputFiles({
    name: 'aia2-local.txt', mimeType: 'text/plain', buffer: Buffer.from(text)
  });
  ...
  assert(after.draft.includes(text), `${profile}: local document did not enter the local draft surface.`);
...
async function exerciseExplanationAndRoutes(page, profile, report) {
  ...
  for (const routeName of ['CUSTODIAL', 'AUDIT', 'IMPLEMENTATION', 'EXPERIENTIAL']) {
    ...
    assert(routed.case_id === before.case_id && routed.draft === before.draft,
      `${profile}/${routeName}: route changed governed work.`);
  ...
  assert(returned.case_id === before.case_id && returned.draft === before.draft,
    `${profile}: recovery changed work.`);
...
async function runJourney(browser, profile, options, report) {
  ...
  await firstUse(page, profile, report);
  await page.screenshot({ path: path.join(artifactDir, `${browserName}-${profile}-first-use.png`), fullPage: true });
  ...
  await page.screenshot({ path: path.join(artifactDir, `${browserName}-${profile}-complete.png`), fullPage: true });
  await context.close();
  return final;
}
...
const report = {
  ...
  production_observation: productionObservation, profiles: {}, steps: [],
  console_errors: [], page_errors: [], http_errors: [], external_requests: [], non_read_requests: [],
  authority: {
...
try {
  ...
  try {
    report.profiles.desktop = await runJourney(browser, 'desktop', {
    ...
    report.profiles.mobile = await runJourney(browser, 'mobile', {
      viewport: { width: 390, height: 844 }, colorScheme: 'dark', reducedMotion: 'no-preference', ...mobile
    }, report);
  } finally {
    await browser.close();
  }
  ...
  await fs.writeFile(path.join(artifactDir, 'ash-aia2-task-continuity.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
}
...
console.log(JSON.stringify({
  ...
  artifact: path.join(artifactDir, 'ash-aia2-task-continuity.json')
}, null, 2));
