...
const profiles = [
  { name:'desktop-fresh', viewport:{ width:1440, height:900 }, isMobile:false, hasTouch:false, stale:false },
  { name:'desktop-stale', viewport:{ width:1440, height:900 }, isMobile:false, hasTouch:false, stale:true },
  { name:'mobile-fresh', viewport:{ width:390, height:844 }, isMobile:browserName === 'webkit', hasTouch:browserName === 'webkit', stale:false },
  { name:'mobile-stale', viewport:{ width:390, height:844 }, isMobile:browserName === 'webkit', hasTouch:browserName === 'webkit', stale:true }
];
...
async function prepareContext(page, stale) {
  await page.goto(`${base}/dome-world/?ash_aia3_prepare=${Date.now()}`, { waitUntil:'domcontentloaded' });
  await page.evaluate(async staleClient => {
    localStorage.clear();
    ...
    sessionStorage.setItem('td613:ash-keep:aia-route:v0.1', 'AUDIT');
    if (globalThis.caches?.open) {
      const cache = await caches.open('td613-retired-aia2-cache');
      await cache.put('/dome-world/ash-keep-aia.js?v=20260720-ak-aia-2-rescue', new Response('/* retired AIA2 fixture */', { headers:{ 'Content-Type':'text/javascript' } }));
    }
    if (navigator.serviceWorker?.register) {
      const registration = await navigator.serviceWorker.register(`/dome-world/ash-stale-client-fixture-sw.js?fixture=${Date.now()}`, { scope:'/dome-world/' });
      await navigator.serviceWorker.ready;
      if (!registration.active) await new Promise(resolve => setTimeout(resolve, 250));
    }
  }, stale);
}
...
async function runProfile(browser, config, report) {
  ...
  await prepareContext(page, config.stale);
  ...
  assert(first.preflight?.indexeddb_preserved === true && first.preflight?.case_data_preserved === true && first.preflight?.active_session_reset === false, `${config.name}: preflight preservation receipt missing.`);
  if (config.stale) {
    assert(first.preflight?.performed === true, `${config.name}: stale preflight did not run.`);
    assert(first.preflight?.cache_names?.includes('td613-retired-aia2-cache'), `${config.name}: retired CacheStorage was not named in eviction receipt.`);
    assert((first.preflight?.worker_scopes || []).some(scope => /\/dome-world\/$/.test(scope)), `${config.name}: retired worker was not named in eviction receipt.`);
  }
  ...
  const mobile = config.name.startsWith('mobile');
  const maxHeight = mobile ? 1050 : 620;
  assert(opened.root.rect.height <= maxHeight, `${config.name}: AIA crown too tall (${opened.root.rect.height}).`);
  assert(opened.main.rect.y <= (mobile ? 1300 : 850), `${config.name}: exact work begins too late (${opened.main.rect.y}).`);
  ...
  assert(returned.cache?.indexeddb_preserved === true && returned.cache?.case_data_preserved === true && returned.cache?.local_case_pointer_preserved === true, `${config.name}: client cache receipt did not preserve cases.`);
...
const report = {
  schema:'td613.ash.aia3-task-continuity-browser-evidence/v0.3-fresh-and-stale',
  status:'RUNNING', browser:browserName, base_url:base,
  source_packet_commit:process.env.TD613_SOURCE_PACKET_COMMIT || null,
  production_observation:process.env.TD613_PRODUCTION_OBSERVATION === 'true',
  profiles:{}, steps:[], console_errors:[], page_errors:[], http_errors:[], external_requests:[], non_read_requests:[],
