...
const plans={
  political_campaign:{title:'Harbor City Mayoral Campaign',route:'route_reporter_response',button:'Political Campaign',entry:'map'},
  fundraiser:{title:'Northstar Arts Benefit',route:'route_lead_host_brief',button:'Fundraiser',entry:'work'}
};
...
async function open(page,name){await page.evaluate(value=>(window.__td613AshUiUxRescue?.open||window.__td613AshPremiumUI?.open||window.__td613OpenAshWorkspace)(value),name);await page.waitForFunction(value=>{const panel=document.getElementById(`workspace-${value}`);const style=panel?getComputedStyle(panel):null;const rect=panel?.getBoundingClientRect();return document.documentElement.dataset.ashPremiumWorkspace===value&&panel?.classList.contains('active')&&style?.display!=='none'&&style?.visibility!=='hidden'&&Number(style?.opacity)>0&&style?.pointerEvents!=='none'&&rect?.width>0&&rect?.height>0;},name);}
async function waitEntry(page,profile,entry){await page.waitForFunction(({value,workspace})=>{const caseId=localStorage.getItem('td613.ash-keep.current-case');const convergence=window.__td613AshDemoEntryConvergence?.current?.()||null;return Boolean(caseId)&&document.documentElement.dataset.ashDemoProfile===value&&document.documentElement.dataset.ashDemoEntryReady===`${value}:${workspace}`&&document.documentElement.dataset.ashDemoEntryCase===caseId&&document.documentElement.dataset.ashDemoEntryHydrating!=='true'&&!document.documentElement.dataset.ashDemoEntryHold&&convergence?.profile===value&&convergence?.workspace===workspace&&convergence?.posture==='READY'&&convergence?.phase==='VISIBLE';},{value:profile,workspace:entry});await open(page,entry);}

await fsp.mkdir(out,{recursive:true});
const browser=await chromium.launch(launch);
const report={schema:'td613.ash.profile-demo-flight/v0.5-entry-converged-map-docket',status:'RUNNING',profiles:{},errors:[],http_errors:[],seams:[]};
try{
  for(const [profile,plan] of Object.entries(plans)){
    ...
    await page.waitForFunction(version=>window.__td613AshProfileDemos?.version===version&&Boolean(window.__td613AshKeep?.version)&&Boolean(window.__td613AshPremiumUI?.version)&&Boolean(window.__td613AshDemoEntryConvergence?.version),METHOD);
    ...
    await waitEntry(page,profile,plan.entry);
    ...
    await page.setViewportSize({width:390,height:844});await open(page,'map');
    ...
    report.profiles[profile]={case_id:state.caseMap.case_id,case_map_digest:state.caseMap.case_map_digest,route_memory_digest:state.routeMemory.route_memory_digest,entry_workspace:plan.entry,...api.counts,mobile};
    await context.close();
  }
