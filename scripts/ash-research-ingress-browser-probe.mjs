...
const report={schema:'td613.ash.research-ingress-flight/v0.1',browser:engineName,status:'RUNNING',launch:null,ingress:[],research:null,external_requests:[],console_errors:[],http_errors:[],seams:[]};
let browser=null;
...
try{
  ...
  for(const viewport of viewports){
    ...
    try{
      ...
      await page.waitForFunction(()=>Boolean(window.__td613AshIngressLayout?.version)&&Boolean(window.__td613AshResearchDemo?.version)&&Boolean(window.__td613AshResearchControlState?.version)&&Boolean(window.__td613AshKeep?.version));
      ...
      if(viewport.name==='desktop-standard'){
        ...
        await page.waitForFunction(()=>{
          ...
          return button && !button.disabled && button.getAttribute('aria-disabled')==='false' && button.dataset.ashResearchControlState==='READY' && /Research qualification/.test(button.textContent||'');
        });
  ...
  terminalError=error;
  report.status='HOLD';
  if(!report.launch) report.launch={status:'HOLD',message:error.message};
  report.seams.push({message:error.message,stack:error.stack});
}finally{
  await fsp.writeFile(path.join(out,`${engineName}-research-ingress-flight.json`),JSON.stringify(report,null,2));
