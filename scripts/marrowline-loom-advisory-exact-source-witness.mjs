...
let fetchedRawHead = false;
let treeEqual = true;
let custodyMode = eventHead ? 'pull-request-whole-tree-parity' : 'non-pr-checkout-tree';
let error = null;
...
try {
  if (eventHead) {
    ...
    treeEqual = checkoutTree === headTree;
  }
} catch (failure) {
  treeEqual = false;
...
const custody = Object.freeze({
  schema: 'td613.marrowline.loom-advisory-source-custody/v0.1-whole-tree-parity',
  browser,
  ...
  raw_head_fetched: fetchedRawHead,
  whole_tree_equal: treeEqual,
  source_bytes_equivalent_to_raw_head: treeEqual,
  raw_head_commit_checked_out: checkoutCommit === headCommit,
  commit_identity_equivalence_claimed: false,
  tree_byte_equivalence_claimed: treeEqual,
  error,
...
const custodyPath = path.join(artifactDir, `marrowline-loom-${browser}-source-custody.json`);
await fs.writeFile(custodyPath, `${JSON.stringify(custody, null, 2)}\n`, 'utf8');
console.log(JSON.stringify(custody, null, 2));

if (!treeEqual) {
  throw new Error(`Marrowline Loom browser witness custody held: tested tree ${checkoutTree} != raw head tree ${headTree}`);
}

const scriptsDir = path.dirname(fileURLToPath(import.meta.url));
const browserWitness = path.join(scriptsDir, 'marrowline-loom-advisory-browser-witness.mjs');
await import(`${pathToFileURL(browserWitness).href}?td613_exact_source_tree=${headTree}&t=${Date.now()}`);
