...
vercel.headers = (vercel.headers || []).filter((entry) => entry.source !== '/api/khonapolit-quality');
...
for (const source of ['/api/hush-generate', '/api/hush-generate-budgeted', '/api/hush-generate-quality', '/api/gemini-readiness', '/api/khonapolit']) {
...
The production deployment at commit \`8b42bc1\` was rejected by the Vercel Hobby-plan ceiling because the repository exposed sixteen deployable files under the root \`api/\` directory while the platform ceiling permits twelve.

## Repair

- Restored the TD613 operating budget to **11 deployable functions** with **1 reserved slot**.
- Moved shared Gemini/Hush/Kʰonapolit implementations and model-policy utilities to \`server/\`.
- Preserved public routes through four thin \`api/\` boundary wrappers.
- Retired \`api/hush-generate-strict-pr124.js\`.
- Removed the redundant \`api/hush-generate.js\` function while retaining its rewrite to \`/api/hush-generate-quality\`.
- Collapsed Kʰonapolit onto the canonical \`/api/khonapolit\` function.
- Strengthened deployment hygiene so CI counts actual deployable \`api/\` files rather than only entries listed in \`vercel.json.functions\`.
...
console.log('Vercel function budget repaired to 11 deployable /api files.');
