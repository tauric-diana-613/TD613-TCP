...
const contentionReplacement = `  const contentionEvent = 'td613:ash:probe-contention-release:v2';
  ...
  const secondResult = await Promise.race([
    secondPage.evaluate(() => window.TD613AshConvergence.withOperation('probe-contention', async () => ({
      state:localStorage.getItem('td613.ash-keep.probe-lock'),
      acquired_at:Date.now()
    }))),
    new Promise((_, reject) => setTimeout(
  ...
  await secondPage.evaluate(key => localStorage.removeItem(key), intentKey);
...
runtimeWrapper = runtimeWrapper.replace(
  ...
  "if (!runtime.includes('Cross-tab lock witness exceeded 35000ms.') || !runtime.includes('First-tab lock release exceeded 10000ms.') || !runtime.includes('DENIED_WHILE_HELD') || !runtime.includes('Second-tab contention intent was not observed before first-tab release.')) {\n  throw new Error('Convergence observer finite cross-tab exclusion witness was not materialized.');\n}"
);
if (!runtimeWrapper.includes("contentionEvent = 'td613:ash:probe-contention-release:v2'")) {
