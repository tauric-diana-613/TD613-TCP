...
const replacement = `  await page.locator('#selectCase').selectOption(secondCase);
  await page.waitForFunction(id => {
    ...
    return select.value === id && remove?.disabled === false;
  }, secondCase);`;

await fs.mkdir(artifactDir, { recursive: true });
const source = await fs.readFile(sourceUrl, 'utf8');
const count = source.split(target).length - 1;
if (count !== 1) throw new Error(`Convergence observer expected one case-selection seam; observed ${count}.`);
const runtime = source.replace(target, replacement);
if (!runtime.includes("select.dispatchEvent(new Event('change', { bubbles: true }))")) {
  throw new Error('Convergence observer repaint-resilient selection was not materialized.');
}
