...
This runner intentionally writes only to SRC live/post-seal paths.  It never mutates
sealed Phase-2 receipts or promotes live captures into A15-R0 scientific authority.

The source identity boundary is the exact Zenodo creator ORCID.  Delta identity is
set difference against SRC records that already have archive metadata custody; a
human-facing date label, record number, or watermark timestamp is never the sole
exclusion criterion.
"""
...
USER_AGENT = "SRC-Zenodo-Operator-Sync/1.0 (+public preservation; operator gated)"
...
def iter_jsonl(path: Path) -> Iterable[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8-sig") as fh:
        for line_no, line in enumerate(fh, 1):
            ...
            except Exception as exc:  # pragma: no cover - defensive custody failure
...
def query_creator_records(orcid: str, max_pages: int) -> list[dict[str, Any]]:
    ...
    for page in range(1, max_pages + 1):
        url = f"https://zenodo.org/api/records?q={query}&sort=newest&size={ZENODO_PAGE_SIZE}&page={page}"
        ...
        raise RuntimeError(f"Zenodo creator query exceeded max_pages={max_pages}; refusing incomplete delta claim")
    return list(found.values())


def collect_captured_record_ids(src_root: Path) -> set[str]:
    """IDs with actual metadata custody, not merely a pending/user-supplied reference."""
    ...
    # Durable derivative presence is independent confirmation of metadata custody.
    patterns = [
...
def extract_text(name: str, data: bytes) -> tuple[str, str]:
    ...
    except Exception as exc:  # extraction failure must remain typed, not fabricated
        return "", f"EXTRACTION_ERROR:{type(exc).__name__}:{exc}"
...
def metadata_markdown(record: dict[str, Any], metadata_sha: str, metadata_blob_path: str) -> str:
    ...
    return (
        "---\n"
        f"schema: src-live-zenodo-metadata-derivative/v1\nrecord_id: \"{rid}\"\n"
...
def body_markdown(record: dict[str, Any], parts: list[dict[str, Any]]) -> str:
    ...
    for part in parts:
        ...
        if text:
            ...
            lines += ["[No body text extracted from this source file.]", ""]
    return "\n".join(lines).rstrip() + "\n"


def record_sort_key(record: dict[str, Any]) -> tuple[str, int]:
    created = str(record.get("created") or "")
    rid = int(record.get("id") or record.get("recid") or 0)
    return created, rid


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", type=Path, required=True, help="SRC root")
    ...
    all_records = query_creator_records(args.creator_orcid, args.max_pages)
    exact_records = [r for r in all_records if exact_creator(r, args.creator_orcid)]
    ...
    for record in new_records:
        ...
        metadata_derivative_rel = Path("03-DERIVATIVES") / "text" / "live" / "zenodo" / f"zenodo-{rid}-metadata.md"
        metadata_derivative = src_root / metadata_derivative_rel
        metadata_derivative.parent.mkdir(parents=True, exist_ok=True)
        metadata_derivative.write_text(metadata_markdown(record, metadata_sha, metadata_blob_path), encoding="utf-8")
        ...
        if public_files_allowed:
            for source_file in record.get("files") or []:
                ...
                url = str((source_file.get("links") or {}).get("self") or "")
                if not url:
                ...
                row = {
                    "source_name": name,
                    "source_url": url,
                    "source_checksum": source_file.get("checksum"),
                    "source_size": source_file.get("size"),
                    "captured_bytes": len(data),
                    "sha256": digest,
                    "blob_path": blob_path,
                    "capture_status": "CAPTURED",
                    "body_extraction_status": extraction_status,
                }
        ...
        body_derivative_rel = Path("03-DERIVATIVES") / "text" / "live" / "zenodo" / f"zenodo-{rid}-body.md"
        body_derivative = src_root / body_derivative_rel
        body_derivative.parent.mkdir(parents=True, exist_ok=True)
        body_derivative.write_text(body_markdown(record, body_parts), encoding="utf-8")

        if any(p.get("body_extraction_status") not in {"PYPDF_TEXT_EXTRACTED", "DOCX_XML_TEXT_EXTRACTED", "TEXT_DECODED", "HTML_TEXT_EXTRACTED"} for p in file_rows if p.get("capture_status") == "CAPTURED"):
        ...
        manifest_rows.append(
            {
                ...
                "title": title,
                "created_utc": record.get("created"),
                "modified_utc": record.get("modified"),
                ...
                "license": license_id,
                "metadata_capture_status": "CAPTURED",
                "metadata_blob_sha256": metadata_sha,
                "metadata_blob_path": metadata_blob_path,
                "metadata_derivative_path": metadata_derivative_rel.as_posix(),
                "body_derivative_path": body_derivative_rel.as_posix(),
                "file_capture_authorized": public_files_allowed,
                "files": file_rows,
                "operator_gate_issue": str(args.gate_issue),
                "operator_relay": args.relay_identity,
                "sync_run_id": args.run_id,
                "archive_authority": "LIVE_POST_SEAL_INTAKE",
                "non_claims": [
                    "capture implies canon admission",
                    "capture implies empirical validation",
                    "publication timing proves causation",
                    "publication timing proves private workspace access",
                ],
            }
        )
    ...
    watermark = {
        ...
        "captured_record_count_before": len(before_ids),
        "creator_records_observed": len(exact_records),
        "new_records_captured": len(captured_ids),
        "delta_method": "SET_DIFFERENCE_AGAINST_METADATA_CUSTODY",
    ...
    run_receipt = {
        ...
        "creator_records_observed": len(exact_records),
        "preexisting_metadata_custody_count": len(before_ids),
        "new_record_count": len(captured_ids),
        "new_record_ids": captured_ids,
