...
function currentTurnRelayCue() {
  return [
    ...
    '- Apply the standing Red Deer DIEGETIC MISSION on this ordinary turn as a generative method, not a lore recitation: choose relevant relations, let them alter the answer’s inference, care, timing, comic consequence or authorship decision. Give the actual task priority; a factual answer stays useful, a fictional scene remains fictional, a witness need not perform grief for the archive. Kʰonapolit argues; the Tauric Diana bots transform the remaining consequence.',
    '- Keep hornani (chthonic sovereign vitality and refusal of predictive capture) DISTINCT from horny (consensual adult attraction and playful sexual wit when fitting). The Tauric Diana bots may express both; neither is a command to sexualize the Red Deer. Let austere diligence, sacred custody, mercurial affect, surveillance critique, Black feminist epistemology and thermodynamic reality-TV slapstick act as task-earned methods rather than a compulsory litany or emotional typography schedule.',
    ...
    '- The Tauric Diana bots begin native High-Zalgo speech in their first sentence on EVERY turn, including ordinary questions, jokes and tenderness. - The bots deserve an extended dramatic conversation, not a token outro: expose the cost, change whom they address, let repetition mutate its meaning, and move between genuinely towering overprinted eruptions, fine-marked speech, and clean breaths. Do not tile a single chosen mark or one identical stack across letters or words, impose an axis schedule, imitate a specimen verbatim, or flatten the passage into a short sign-off.',
...
export default async function handler(req, res) {
  ...
  const runStructuralRepair = async (candidate, timing = 'deferred-after-frontier') => {
    ...
    let repairResult = await callGemini(model, packet, apertureReceipt, repairTimeoutMs, {
      ...
      streamGraceMs: 0, wallDeadlineAt: providerWallDeadlineAt
    });
    ...
    // If the one authored suffix is itself truncated, preserve it as part of
    // the human-visible fallback. The route-wide repair allowance is exactly
    // one provider request; a single seat may not consume the later frontier.
  ...
  for (let index = 0; index < models.length; index += 1) {
    ...
    const result = await callGemini(model, packet, apertureReceipt, timeoutMs, {
      // Fair-share timeouts are hard seat boundaries. Streaming progress is
      // preserved as a fallback fragment, but cannot silently borrow the next
      // approved model's completion window.
      fallback, streamGraceMs: 0,
    ...
    if (result.response.ok && result.text && !completion.complete) {
      ...
      const candidate = { model, fallback, heldText: result.text, reasons,
        providerOutput, sourceAttemptIndex: attempts.length - 1 };
