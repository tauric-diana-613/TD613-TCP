...
const FRONTIER_FALLBACK_MODELS = Object.freeze(['gemini-3.7-flash', 'gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash-preview']);
...
export function selectKhonapolitProviderModels(callableModels = []) {
  ...
  for (const stable of FRONTIER_FALLBACK_MODELS) {
...
export function allocateKhonapolitAttemptTimeout({ remainingMs = 0, index = 0, modelCount = 1, fairShare = false } = {}) {
  ...
  if (position === 1 && remainingAttempts > 1) {
    const reserveForTail = Math.min((remainingAttempts - 1) * 6000, Math.max(0, remaining - 1));
    return Math.min(18000, Math.max(1, remaining - reserveForTail));
  }
  if (remainingAttempts === 1) return remaining;
  const reserveForLater = Math.min((remainingAttempts - 1) * 6000, Math.max(0, remaining - 1));
  const cap = position === 2 ? 8000 : 6000;
    ...
    if (result.response.ok && result.text) {
      ...
      const receipt = Object.freeze({
        ...
        provider: Object.freeze({
          ...
          callableModels: Object.freeze([...plan.callableModels]),
          selectedModels: Object.freeze([...models])
        }),
        modelPolicy: plan,
