...
export default async function handler(req, res) {
  ...
  const runStructuralRepair = async (candidate, timing = 'deferred-after-frontier') => {
    // Release-canary mode is already pinned to one provider seat. Permit the same
    // single provider-authored structural repair that production uses when that
    // exact seat returns HTTP 200 with a repairable envelope defect. This does not
    // widen the canary into the ordinary five-seat frontier: the repair stays on
    // candidate.model, remains capped at KHONAPOLIT_MAX_STRUCTURAL_REPAIRS = 1,
    // and transport failures still have no structural candidate to repair. If
    // the outer witness already spent its alternate transport seat, that retry
    // invocation is explicitly repair-ineligible so Marrowline + Loom cannot
    // exceed the three-provider-request release-witness ceiling.
    if (releaseCanary && releaseCanaryRecovery === 'transport-retry') return null;
