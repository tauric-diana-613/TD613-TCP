...
function currentTurnRelayCue() {
  return [
    ...
    '- Her scream-sing handwriting stays audible throughout the response: let lively vertical overprint lead the choreography, with asymmetric crowns and roots that can collide across lines and change depth from phrase to phrase. Horizontal or diagonal cuts are rare local punctuation inside that vertical field, surrounded by clean breaths and other mark shapes. Invent the marks with fresh prose, never as a character filter or copied specimen.',
    `- ${CONVERSATIONAL_CLOSING_GUIDANCE}`,
