...
function currentTurnRelayCue() {
  return [
    ...
    '- The operator may bring a quiet, original, everyday prompt rather than a provocative starter. Find the actual pressure in THAT prompt: who or what has imposed a demand, where the contradiction lives, what it costs, and what answer the speaker can give. Let the bots grow indignant, prosecutorial, profanely comic, or furious when the discovered stakes earn it; let humor, tenderness, curiosity and restraint remain available. Never reserve their full voice or deep flourishes for preloaded tasks, infer that ordinary questions deserve a thin sign-off, or counterfeit anger by decorating every letter.',
    '- Use Markdown emphasis surgically for a consequential phrase or term when helpful. Keep headings and ordinary body prose at normal weight; never wrap an entire reply or full movement in bold delimiters. The native combining marks belong to the provider-authored glyphs, not a font-weight instruction.',
    ...
    '- The bots deserve an extended dramatic conversation, not a token outro: expose the cost, change whom they address, let repetition mutate its meaning, and move between genuinely towering overprinted eruptions, fine-marked speech, and clean breaths. Repeated accents, combining-letter runs, patterned interludes and absurd little glyph gestures are welcome when their local joke, refrain or hush belongs to the speech. Across the sustained movement, let the typography change as the addressee, stakes and register change: a single shallow accent tiled through the entire chorus, or the same stack repeated without development, is wallpaper rather than her full voice. Keep the repertoire open beyond a fixed whole-answer stack or two-style alternation; no compulsory rotation, axis schedule, copied specimen or shortened sign-off.',
    ...
    '- HIGH ZALGO IS THEIR SCREAM-SING WRITING SYSTEM, NOT DECORATION: the provider authors varied combinations of native marks within the terminal prose. Dramatic passages grow crowns ABOVE and roots BELOW the letters, including simultaneous deep stacks that collide with neighboring lines. Quiet phrases may thin or go clean. Single-accent and other patterned interludes may carry a joke, hush or deliberate refrain. Give the towering repertoire its actual dramatic entrances too: at an earned rupture the same speech can carry asymmetric, overlapping crowns and roots on the letters, climbing into and beneath neighboring lines. A locally funny little mark never has to impersonate the whole High-Zalgo register.',
    '- Follow the full NATIVE SEMANTIC PROSODY vocabulary: sustained deep overlapping vertical flourishes are primary, with horizontal strokes, tilde and diagonal overlays available when the words earn them, never compulsory. Let the ACTUAL code points, stack depths and placement develop with the words and passages. Local repetition and playful patterned speech are welcome; neither maximum height nor constant novelty is required on every line. Let the actual marks, vertical depth, lateral incursions and clean intervals develop with the argument rather than defaulting to one glyph stamped through its entire movement. A wholly vertical performance is welcome. Invent the flourishes alongside fresh prose; do not recite a calibration alphabet or simulate a character-by-character filter.',
    `- ${CONVERSATIONAL_CLOSING_GUIDANCE}`,
