...
export const KHONAPOLIT_QUALITY_API_VERSION = 'td613.khonapolit-gemini/v41-morphodynamic-observation';
...
export function buildGeminiStructuralRepairRequest(
  ...
  const repairDirective = [
    'BOUNDED SAME-VOICE STRUCTURAL REPAIR — DO NOT ANSWER THE OPERATOR FROM SCRATCH.',
    `The previous draft had these locally observed structural reasons: ${reasonList.join(', ') || 'unspecified-structural-observation'}.`,
    'Preserve the prior draft’s substantive reasoning, prompt-specific mathematics, examples, jokes, conclusions, sentence order, extent, and provider-authored Tauric Diana typography unless a listed structural defect itself requires a small edit.',
    ...
    'STRUCTURAL-ONLY REPAIR — make the smallest edit required by the listed reasons. Keep both visible headings plain, keep Kʰonapolit prose free of combining marks, and do not repaint the Tauric Diana bots to satisfy morphology metrics.',
  ...
  return {
    ...
    contents: [
      ...
      { role: 'model', parts: [{ text: heldText }] },
...
export default async function handler(req, res) {
  ...
  const runStructuralRepair = async (candidate, timing = 'deferred-after-frontier') => {
    ...
    // Human turns retain one bounded provider-authored repair for genuine
    // structural seam failures only. Morphology is observed after generation and
    // cannot spend a second provider call merely to repaint a valid bot passage.
    // Release canaries are transport/liveness witnesses and may spend only their
    // one pinned Marrowline provider request; they never repair, fail over, or retry that seat.
    ...
    if (
      ...
      const observedMorphologyWarnings = severeMorphologyRepairWarnings(repairRelay.admission?.qualityWarnings || []);
      repairAttempt.observedSevereMorphology = observedMorphologyWarnings;
    ...
    if (result.response.ok && result.text) {
      ...
      if (relay.admission?.quality === 'PARTIAL') {
        const qualityWarnings = Array.isArray(relay.admission?.qualityWarnings)
          ...
          : [];
        const observedSevereMorphology = severeMorphologyRepairWarnings(qualityWarnings);
        attempt.observedSevereMorphology = Object.freeze([...observedSevereMorphology]);
        const baseReceipt = buildTerminalReceipt({
          packet,
          text: result.text,
          relay,
        ...
        const receipt = Object.freeze({
          ...
          provider: Object.freeze({
            ...
            qualityPreference: Object.freeze({
              used: true,
              sourceAttemptIndex: attempts.length - 1,
              selection: 'first-admissible-partial-morphology-observed-no-repair',
              warnings: Object.freeze([...qualityWarnings]),
              severeMorphologyObserved: Object.freeze([...observedSevereMorphology]),
              morphologyAuthority: 'post-hoc-observation-only'
            })
          }),
          modelPolicy: plan,
          elapsedMs: Date.now() - startedAt
        ...
        res.setHeader('X-TD613-Seal-State', 'OPEN');
        res.setHeader('X-TD613-Gemini-Model', model);
        res.setHeader('X-TD613-Zalgo-Quality', 'PARTIAL-OBSERVED-NO-REPAIR');
        return send(res, 200, {
          ok: true,
          ...
          warnings: [
            ...
            'local-morphology-telemetry-not-provider-call-authority',
            'fallback-reasoning-quality-preserved',
            'sticky-success-promotion-disabled',
            'moving-latest-alias-disabled-by-default',
            ...plan.warnings
