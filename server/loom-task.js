  ...
  return async function loomTaskHandler(req, res) {
    ...
    try {
      ...
      const canaryModel = requestedCanaryModel && allModels.includes(requestedCanaryModel)
        ...
        : allModels[0] || null;
      const canaryModels = canaryModel ? [canaryModel] : [];
      // Release witnessing must not consume the interactive frontier. One pinned Loom
      // seat answers one production-liveness question; provider transport failure is
      // recorded as HELD and may later use the zero-deploy re-observation conduit.
