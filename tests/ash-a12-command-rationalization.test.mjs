...
assert.match(browserWrapper, /rebind_required:!already_converged/);
assert.match(browserWrapper, /const rebind_admitted = Boolean\(began\) \|\| \(already_converged && after_converged\)/);
assert.match(browserWrapper, /if \(!entry_convergence_rebind\.rebind_admitted\)/);
...
console.log('Ash A12 command rationalization contract passed under diagnostic-first entry preflight, idempotent present-state convergence, post-click settlement, canonical-field settlement, and stateful failure diagnostics.');
