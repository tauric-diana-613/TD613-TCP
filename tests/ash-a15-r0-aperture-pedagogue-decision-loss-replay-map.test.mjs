...
const expected=[[0.50,0.40,'P_ORTH'],[0.50,0.55,'P_ORTH'],[0.50,0.70,'P_DIAG'],[0.546918160706758,0.40,'P_ORTH'],[0.546918160706758,0.55,'P_DIAG'],[0.546918160706758,0.70,'P_DIAG'],[0.60,0.40,'P_ORTH'],[0.60,0.55,'P_DIAG'],[0.60,0.70,'P_DIAG']];
assert.deepEqual(map.cells.map(cell=>[cell.rho,cell.s,cell.selected_probe_id]),expected);
assert.deepEqual(receipt.decision_state_coordinates,['measurement_posture','decision_consequence_specification','question_selection','replay_sensitivity']);
...
console.log(JSON.stringify({ok:true,schema:receipt.schema,analytic_checkpoints:receipt.analytic_checkpoints,decision_specification_boundary:{analytic:decision.analytic_boundary_s,numeric:decision.numeric_boundary,error:decision.absolute_boundary_error},measurement_model_boundary:{analytic:measurement.analytic_boundary_rho,numeric:measurement.numeric_boundary,error:measurement.absolute_boundary_error},stable_interiors:{decision_low:interiors.decision_low_stable,decision_high:interiors.decision_high_stable,measurement_low:interiors.measurement_low_stable,measurement_high:interiors.measurement_high_stable},joint_map:map.cells.map(cell=>({rho:cell.rho,s:cell.s,selected_probe_id:cell.selected_probe_id})),next_learning_action:receipt.next_learning_action,promotion_authority:receipt.promotion_authority},null,2));

await import('./ash-a15-r0-aperture-pedagogue-replay-sensitivity-gate.test.mjs');
