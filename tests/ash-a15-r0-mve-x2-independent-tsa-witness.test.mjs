...
assert.equal(pilot.pinned_external_tsa_certificate_fingerprints_verified,true);
...
assert.ok(Math.abs(pilot.bounded_conditional_origin_information_bits-1)<1e-12,'Balanced external TSA witness separation should carry one bounded route bit.');
assert.equal(pilot.independently_governed_external_witness_acquired,true);
assert.equal(pilot.empirical_exogenous_channel_acquired,true);
assert.equal(pilot.bounded_empirical_exteriority_information_gain_measured,true);
...
assert.equal(earned.status,'MVE_X2_INDEPENDENT_RFC3161_WITNESS_EARNED');
assert.equal(earned.rest_symbol,'𝄐');
assert.equal(earned.independently_governed_external_witness_acquired,true);
assert.equal(earned.empirical_exogenous_channel_acquired,true);
assert.equal(earned.signed_external_witness_material_preserved_in_same_run_custody,true);
assert.equal(earned.bounded_empirical_exteriority_information_gain_measured,true);
assert.equal(earned.empirical_exteriority_scope,'BOUNDED_ROUTE_WITNESS_ONLY');
...
for(const [label,mutate] of [
  ...
  ['unpinned cert',copy=>{copy.certificate_fingerprints_preregistered=false;}],
...
assert.equal(adjudicateMveX2Pilot({...pilot,pinned_external_tsa_certificate_fingerprints_verified:false}).status,'INADMISSIBLE');
assert.equal(adjudicateMveX2Pilot({...pilot,signed_receipts_preserved_in_same_run_custody:false}).status,'INADMISSIBLE');
assert.equal(adjudicateMveX2Pilot({...pilot,a_plus_x_origin_accuracy:0.5,bounded_conditional_origin_information_bits:0}).status,'INADMISSIBLE');
...
console.log('MVE-X2 independently administered RFC3161 witness tests passed.');
