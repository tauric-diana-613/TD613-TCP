...
const SUEZ_SOURCE_COMMIT='d652c5e151471be7e40ff6a08936ba26c0cef1ad';
const SUEZ_DERIVATIVE_PATH='packages/dome_world_exact/fixtures/a15-r0/SRC/03-DERIVATIVES/text/zenodo/zenodo-22019413-file-0.md';
...
assert.ok([9,10,11].includes(ahead),'Golden Egg co-observation chamber must remain within frozen or documented post-RED custody-repair depth');
...
const allowed=new Set([
  ...
  'app/dome-world/docs/ash/experiments/a15-r0/GOLDEN_EGG_COOBSERVATION_ADMISSIBILITY_FREEZE_V0_1.md',
...
assert.equal(changed.length,8,'post-RED custody repair may not widen the eight-path frozen chamber');
for(const p of allowed)assert.equal(changed.includes(p),true,`missing frozen co-observation path: ${p}`);
...
for(const [path,expected] of parentSourceBlobs){const actual=execFileSync('git',['rev-parse',`${PARENT_986_RECEIPT}:${path}`],{encoding:'utf8'}).trim();assert.equal(actual,expected,`science-parent source blob moved: ${path}`);}
execFileSync('git',['cat-file','-e',`${SUEZ_SOURCE_COMMIT}^{commit}`],{stdio:'pipe'});
const suezActual=execFileSync('git',['rev-parse',`${SUEZ_SOURCE_COMMIT}:${SUEZ_DERIVATIVE_PATH}`],{encoding:'utf8'}).trim();
assert.equal(suezActual,SUEZ_DERIVATIVE_BLOB,'retained external Suez evidence blob moved at its explicit source commit');
