...
// Mechanically replay the exact earned #971 hardening theorem tail while replacing only #971's ancestor-specific path audit.
const parentHardening=execFileSync('git',['show',`${PARENT_971_RECEIPT}:tests/ash-a15-r0-review-hardening.test.mjs`],{encoding:'utf8'});
const parentMarker="// Mechanically replay the exact earned #969 hardening theorem tail while replacing only #969's ancestor-specific path audit.";
const replayStart=parentHardening.indexOf(parentMarker);
assert.ok(replayStart>=0,'earned #971 hardening theorem tail must remain discoverable for descendant replay');
const parentTail=parentHardening.slice(replayStart);
const testsDir=path.dirname(fileURLToPath(import.meta.url));
const tempPath=path.join(testsDir,`.ash-a15-r0-parent-971-hardening-tail-${process.pid}.mjs`);
...
try{await import(`${pathToFileURL(tempPath).href}?parent971=${Date.now()}`);}finally{await fs.rm(tempPath,{force:true});}
