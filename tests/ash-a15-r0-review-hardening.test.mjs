...
const allowedCurrentChamberPaths = new Set([
  'app/dome-world/docs/ash/experiments/a15-r0/APERTURE_PEDAGOGUE_DROMOLOGICAL_HOLONOMY_FAULT_BUDGET_STRATIGRAPHY_AIA_STARTUP_V0_1.md',
  'app/dome-world/previews/a15-r0/dromological-holonomy-fault-budget-stratigraphy-aia.js',
  'tests/ash-a15-r0-aperture-pedagogue-dromological-holonomy-fault-budget-stratigraphy-aia.test.mjs',
...
assert.deepEqual(
  historicalMutations,
  [],
  `post-#830 fault-budget stratigraphy chamber may not mutate inherited A15-R0 paths: ${historicalMutations.join(', ')}`,
);
assert.equal(
  changedA15R0.length,
  allowedCurrentChamberPaths.size,
  `fault-budget stratigraphy chamber must contain exactly ${allowedCurrentChamberPaths.size} live paths; observed ${changedA15R0.length}`,
);
for (const path of allowedCurrentChamberPaths) {
  assert.equal(changedA15R0.includes(path), true, `missing preregistered fault-budget stratigraphy path: ${path}`);
}
...
await import('./ash-a15-r0-aperture-pedagogue-dromological-holonomy-fault-budget-stratigraphy-aia.test.mjs');
await import('./ash-a15-r0-wedding-identifiability.test.mjs');

console.log('Ash A15-R0 dromological holonomy fault-budget stratigraphy AIA hardening tests passed.');
