...
const changedA15R0 = execFileSync(
  'git',
  ['diff', '--name-only', `${PARENT_852_RECEIPT}..HEAD`, '--',
...
const allowedCurrentChamberPaths = new Set([
  'app/dome-world/docs/ash/experiments/a15-r0/APERTURE_PEDAGOGUE_CLAIM_BUNDLE_MINIMAL_SUFFICIENT_CUSTODY_FRONTIER_STARTUP_V0_1.md',
  'app/dome-world/docs/ash/experiments/a15-r0/CLAIM_BUNDLE_MINIMAL_SUFFICIENT_CUSTODY_FRONTIER_STARTUP_HOOK_V0_1.json',
  'app/dome-world/previews/a15-r0/claim-bundle-minimal-sufficient-custody-frontier.js',
  'tests/ash-a15-r0-aperture-pedagogue-claim-bundle-minimal-sufficient-custody-frontier.test.mjs',
  'tests/ash-a15-r0-review-hardening.test.mjs',
]);
const historicalMutations = changedA15R0.filter(path => !allowedCurrentChamberPaths.has(path));
assert.deepEqual(historicalMutations, [], `post-#852 claim-bundle custody chamber may not mutate inherited A15-R0 paths: ${historicalMutations.join(', ')}`);
assert.equal(changedA15R0.length, allowedCurrentChamberPaths.size,
  `claim-bundle custody chamber must contain exactly ${allowedCurrentChamberPaths.size} live paths; observed ${changedA15R0.length}`);
for (const path of allowedCurrentChamberPaths) assert.equal(changedA15R0.includes(path), true, `missing preregistered claim-bundle custody path: ${path}`);
...
console.log('Ash A15-R0 claim-bundle minimal sufficient custody frontier hardening tests passed.');
