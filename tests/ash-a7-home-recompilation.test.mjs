...
for (const label of [
  'What needs attention','Why it matters','What you can do now','What Ash will change','What Ash will not do','Done when',
  'What remains attached','What became stale','What can be returned to','What must be checked again','What has not been sealed',
  'What has already left','What stayed local','Later stale status'
...
assert.match(shell, /ash-a7-a11-recompiler-core\.js\?v=\$\{ASH_LIFECYCLE_ASSET_EPOCH\}/);
assert.match(shell, /ash-a7-home-recompilation\.js\?v=\$\{ASH_LIFECYCLE_ASSET_EPOCH\}/);
