...
const ashWorkflow = fs.readFileSync(new URL('../.github/workflows/ash-flowcore-live-field.yml', import.meta.url), 'utf8');
...
for (const marker of [
  ...
  'Direction: Object A → Object B.','Evidence/source','Commit relationship','Relationship inspection','Graph view',
...
assert.equal((ashWorkflow.match(/run-ash-constitutional-convergence-handshake\.mjs/g) || []).length >= 2, true);
assert.match(ashWorkflow, /node --check scripts\/run-ash-constitutional-convergence-handshake\.mjs/);
...
console.log(JSON.stringify({
  ok:true,schema:'td613.ash.a8-case-map-contract/v0.1',existing_map_engine_delegation:true,lifecycle_hold_receipt_observed:true,
  held_note_write:false,inert_owner_event_delegation:true,disabled_owner_respected:true,stage_form_draft_preservation:true,
