...
for (const token of [
  'td613.ash.a8-dirty-draft-recompile-guard/v0.6-focus-scoped-detached-precedence',
  'td613.ash.a8-dirty-draft-guard-receipt/v0.12-focus-scoped-detached-precedence',
  "const FORM_SELECTOR = '#ashA8ObjectForm,#ashA8RelationForm'","const COMMIT_SELECTOR = '#ashA8CommitObject,#ashA8CommitRelation'",
  'const pendingConnectedIntents = new Map()','let connectedIntentSerial = 0','let connectedPairValuesRestored = 0','let pendingIntentDetachedRestores = 0','let focusScopedIntentsCleared = 0',
  'function snapshotControlIntent(target)','source_target:target','function applyControlIntent(intent, { requireReplacement = false, sourceTarget = null } = {})','function clearPendingConnectedIntentsExcept(controlId = null)','function takePendingConnectedIntent(id)','function rememberConnectedInput(target)','function restoreConnectedChange(target)','function transplantDetachedEdit(target)',
  "reason:'UNIDENTIFIED_CONTROL'","reason:'LIVE_REPLACEMENT_UNAVAILABLE'","reason:'CONTROL_SHAPE_MISMATCH'","reason:'INTENDED_OPTION_UNAVAILABLE'","reason:'LIVE_REPLACEMENT_REJECTED_VALUE'","reason:'NO_PENDING_INPUT_INTENT'","reason:'ORIGINAL_CONTROL_STILL_AUTHORITATIVE'",
  'pendingConnectedIntents.set(intent.id, intent)','clearPendingConnectedIntentsExcept(intent.id)','const intent = takePendingConnectedIntent(id)','const pendingIntent = takePendingConnectedIntent(id)','const intent = pendingIntent || snapshotControlIntent(target)','intent.source_target === target','intent.source_target?.isConnected === true','applyControlIntent(intent, { requireReplacement:true, sourceTarget:intent.source_target })','applyControlIntent(intent, { requireReplacement:true, sourceTarget:pendingIntent?.source_target || target })',
  "event.type === 'focusin'","event.type === 'change'","event.type === 'input'",'clearPendingConnectedIntentsExcept(controlId)','restoreConnectedChange(target)','rememberConnectedInput(target)','CONNECTED_EDIT_PAIR_VALUE_RESTORED','focus_scoped_intent:true',
  'STALE_DETACHED_EDIT_VALUE_TRANSPLANTED','event.stopImmediatePropagation?.()','intent_source:continuity.intent_source || null',"intent_source:pendingIntent ? 'FOCUS_SCOPED_CONNECTED_INPUT' : 'DETACHED_EVENT_SNAPSHOT'",
  'host?.__td613AshA8MapReturnHandshake?.capture?.()','dirtyDraftActive = true','connectedPairValuesRestored += 1','staleDetachedValuesTransplanted += 1','pendingIntentDetachedRestores += 1','focusScopedIntentsCleared += cleared',
  'pending_connected_intents:pendingConnectedIntents.size','connected_pair_values_restored:connectedPairValuesRestored','pending_intent_detached_restores:pendingIntentDetachedRestores','focus_scoped_intents_cleared:focusScopedIntentsCleared',"stale_event_policy:'PREFER_FOCUS_SCOPED_CONNECTED_INPUT_THEN_QUARANTINE_DETACHED_CHANGE'",
  'function admitCommit(event)','pendingConnectedIntents.clear()','COMMIT_GESTURE_ADMITTED',
  'function recoverAfterStageSync(source, detail = {})','DIRTY_DRAFT_RECOVERED_AFTER_STAGE_SYNC','POST_STAGE_SYNC_PRE_GENERIC_DRAFT_RESTORE','authoritative_restore_owner:\'A8_MAP_RETURN_HANDSHAKE_SHADOW\'',
  'function recoverAfterInFlightRecompile(event)','queueMicrotask(() => completeQueuedRecovery(serial, source))','function shouldDefer(source)',"return 'DIRTY_STAGE_DRAFT'",
  ...
  '__td613AshA8RecompileGuard','authority_changed:false','source_bytes_moved:false','human_closure_required:true'
]) assert.ok(dirtyGuard.includes(token), `A8 focus-scoped detached-precedence guard missing ${token}`);
assert.match(dirtyGuard, /function rememberConnectedInput\(target\)[\s\S]{0,520}clearPendingConnectedIntentsExcept\(intent\.id\)[\s\S]{0,260}pendingConnectedIntents\.set\(intent\.id, intent\)/, 'A8 must preserve the authored input intent for the active control focus transaction.');
assert.doesNotMatch(dirtyGuard.match(/function rememberConnectedInput\(target\)[\s\S]*?\n\}/)?.[0] || '', /queueMicrotask|setTimeout|setInterval/, 'A8 input intent must be bounded by focus/change ownership rather than a timing guess.');
assert.match(dirtyGuard, /function restoreConnectedChange\(target\)[\s\S]{0,640}takePendingConnectedIntent\(id\)[\s\S]{0,420}intent\.source_target === target[\s\S]{0,420}applyControlIntent\(intent, \{ requireReplacement:true, sourceTarget:intent\.source_target \}\)/, 'A8 must restore intent only into a different connected replacement node.');
assert.match(dirtyGuard, /function transplantDetachedEdit\(target\)[\s\S]{0,300}takePendingConnectedIntent\(id\)[\s\S]{0,220}pendingIntent \|\| snapshotControlIntent\(target\)[\s\S]{0,420}sourceTarget:pendingIntent\?\.source_target \|\| target[\s\S]{0,420}pendingIntentDetachedRestores \+= 1/, 'A detached change must consume the focus-scoped connected input receipt before consulting its stale detached snapshot.');
assert.match(dirtyGuard, /function beginDirtyDraft\(event\)[\s\S]{0,1500}target\.isConnected !== true[\s\S]{0,360}transplantDetachedEdit\(target\)[\s\S]{0,420}intent_source:continuity\.intent_source[\s\S]{0,520}event\.type === 'focusin'[\s\S]{0,280}clearPendingConnectedIntentsExcept\(controlId\)[\s\S]{0,360}event\.type === 'change'[\s\S]{0,300}restoreConnectedChange\(target\)[\s\S]{0,360}event\.type === 'input'[\s\S]{0,240}rememberConnectedInput\(target\)[\s\S]{0,500}__td613AshA8MapReturnHandshake\?\.capture/, 'A8 must arbitrate detached precedence, replacement change, and focus departure before recapturing the shared shadow.');
assert.match(dirtyGuard, /if \(target\.isConnected !== true\)[\s\S]{0,220}event\.stopImmediatePropagation\?\.\(\)[\s\S]{0,220}transplantDetachedEdit\(target\)[\s\S]{0,480}return continuity\.transplanted/, 'Detached A8 events must remain quarantined while preserving authored intent through the live replacement owner.');
