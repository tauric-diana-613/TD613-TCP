...
for (const token of [
  'td613.ash.a8-dirty-draft-recompile-guard/v0.4-connected-edit-pair-continuity',
  'td613.ash.a8-dirty-draft-guard-receipt/v0.10-connected-edit-pair-continuity',
  "const FORM_SELECTOR = '#ashA8ObjectForm,#ashA8RelationForm'","const COMMIT_SELECTOR = '#ashA8CommitObject,#ashA8CommitRelation'",
  'const pendingConnectedIntents = new Map()','let connectedIntentSerial = 0','let connectedPairValuesRestored = 0',
  'function snapshotControlIntent(target)','function applyControlIntent(intent, { requireReplacement = false, sourceTarget = null } = {})','function rememberConnectedInput(target)','function restoreConnectedChange(target)','function transplantDetachedEdit(target)',
  "reason:'UNIDENTIFIED_CONTROL'","reason:'LIVE_REPLACEMENT_UNAVAILABLE'","reason:'CONTROL_SHAPE_MISMATCH'","reason:'INTENDED_OPTION_UNAVAILABLE'","reason:'LIVE_REPLACEMENT_REJECTED_VALUE'","reason:'NO_PENDING_INPUT_INTENT'",
  'pendingConnectedIntents.set(intent.id, intent)','pendingConnectedIntents.get(intent.id)?.serial === intent.serial','pendingConnectedIntents.delete(id)','queueMicrotask(() => {',
  "event.type === 'change'","event.type === 'input'",'restoreConnectedChange(target)','rememberConnectedInput(target)','CONNECTED_EDIT_PAIR_VALUE_RESTORED','connected_edit_pair:connectedContinuity.transplanted',
  ...
  'host?.__td613AshA8MapReturnHandshake?.capture?.()','dirtyDraftActive = true','connectedPairValuesRestored += 1','staleDetachedValuesTransplanted += 1',
  'pending_connected_intents:pendingConnectedIntents.size','connected_pair_values_restored:connectedPairValuesRestored',"stale_event_policy:'PRESERVE_INPUT_INTENT_ACROSS_CONNECTED_OR_DETACHED_CHANGE_THEN_QUARANTINE_STALE'",
...
]) assert.ok(dirtyGuard.includes(token), `A8 connected-pair guard missing ${token}`);
assert.match(dirtyGuard, /function rememberConnectedInput\(target\)[\s\S]{0,520}pendingConnectedIntents\.set\(intent\.id, intent\)[\s\S]{0,520}queueMicrotask/, 'A8 must preserve the authored input intent through the synchronous input/change turn.');
assert.match(dirtyGuard, /function restoreConnectedChange\(target\)[\s\S]{0,520}pendingConnectedIntents\.get\(id\)[\s\S]{0,520}applyControlIntent\(intent, \{ sourceTarget:target \}\)/, 'A8 must restore pending intent into a connected replacement before change propagation.');
assert.match(dirtyGuard, /function beginDirtyDraft\(event\)[\s\S]{0,1100}event\.type === 'change'[\s\S]{0,420}restoreConnectedChange\(target\)[\s\S]{0,420}event\.type === 'input'[\s\S]{0,260}rememberConnectedInput\(target\)[\s\S]{0,500}__td613AshA8MapReturnHandshake\?\.capture/, 'A8 must arbitrate connected input/change continuity before recapturing the shared shadow.');
