...
for (const token of [
  'td613.ash.a8-dirty-draft-recompile-guard/v0.5-focus-scoped-edit-pair-continuity',
  'td613.ash.a8-dirty-draft-guard-receipt/v0.11-focus-scoped-edit-pair-continuity',
  "const FORM_SELECTOR = '#ashA8ObjectForm,#ashA8RelationForm'","const COMMIT_SELECTOR = '#ashA8CommitObject,#ashA8CommitRelation'",
  'const pendingConnectedIntents = new Map()','let connectedIntentSerial = 0','let connectedPairValuesRestored = 0','let focusScopedIntentsCleared = 0',
  'function snapshotControlIntent(target)','source_target:target','function applyControlIntent(intent, { requireReplacement = false, sourceTarget = null } = {})','function clearPendingConnectedIntentsExcept(controlId = null)','function rememberConnectedInput(target)','function restoreConnectedChange(target)','function transplantDetachedEdit(target)',
  ...
  'pendingConnectedIntents.set(intent.id, intent)','clearPendingConnectedIntentsExcept(intent.id)','pendingConnectedIntents.delete(id)','intent.source_target === target','intent.source_target?.isConnected === true','applyControlIntent(intent, { requireReplacement:true, sourceTarget:intent.source_target })',
  ...
  'applyControlIntent(intent, { requireReplacement:true, sourceTarget:target })','STALE_DETACHED_EDIT_VALUE_TRANSPLANTED','event.stopImmediatePropagation?.()',
  'host?.__td613AshA8MapReturnHandshake?.capture?.()','dirtyDraftActive = true','connectedPairValuesRestored += 1','staleDetachedValuesTransplanted += 1','focusScopedIntentsCleared += cleared',
  'pending_connected_intents:pendingConnectedIntents.size','connected_pair_values_restored:connectedPairValuesRestored','focus_scoped_intents_cleared:focusScopedIntentsCleared',"stale_event_policy:'PRESERVE_INPUT_INTENT_UNTIL_REPLACEMENT_CHANGE_OR_FOCUS_DEPARTURE_THEN_QUARANTINE_STALE'",
...
]) assert.ok(dirtyGuard.includes(token), `A8 focus-scoped connected-pair guard missing ${token}`);
...
assert.match(dirtyGuard, /function restoreConnectedChange\(target\)[\s\S]{0,640}pendingConnectedIntents\.get\(id\)[\s\S]{0,420}intent\.source_target === target[\s\S]{0,420}applyControlIntent\(intent, \{ requireReplacement:true, sourceTarget:intent\.source_target \}\)/, 'A8 must restore intent only into a different connected replacement node.');
assert.match(dirtyGuard, /function beginDirtyDraft\(event\)[\s\S]{0,1300}event\.type === 'focusin'[\s\S]{0,280}clearPendingConnectedIntentsExcept\(controlId\)[\s\S]{0,360}event\.type === 'change'[\s\S]{0,300}restoreConnectedChange\(target\)[\s\S]{0,360}event\.type === 'input'[\s\S]{0,240}rememberConnectedInput\(target\)[\s\S]{0,500}__td613AshA8MapReturnHandshake\?\.capture/, 'A8 must preserve intent until replacement change or focus departure before recapturing the shared shadow.');
...
assert.match(dirtyGuard, /if \(target\.isConnected !== true\)[\s\S]{0,220}event\.stopImmediatePropagation\?\.\(\)[\s\S]{0,220}transplantDetachedEdit\(target\)[\s\S]{0,420}return continuity\.transplanted/, 'Detached A8 events must remain quarantined while preserving authored intent through the live replacement owner.');
