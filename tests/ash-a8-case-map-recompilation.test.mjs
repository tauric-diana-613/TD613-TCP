...
for (const token of [
  ...
  "const COMMIT_SELECTOR = '#ashA8CommitObject,#ashA8CommitRelation'",
  'let dirtyDraftActive = false','let recoveredInFlightRecompiles = 0','function canonicalMapWorkshopIsActive()','function custodyHoldIsActive()',
  ...
  'function recoverAfterInFlightRecompile(event)','host?.__td613AshA8MapReturnHandshake?.restore?.()','DIRTY_DRAFT_RECOVERED_AFTER_IN_FLIGHT_RECOMPILE','DIRTY_DRAFT_RECOVERY_HELD','POST_CORE_GENERIC_DRAFT_RESTORE','full_control_parity:recovered',
  ...
  '__td613AshA8RecompileGuard','shouldDefer','recover:recoverAfterInFlightRecompile','dirty_draft_active:dirtyDraftActive','recovered_in_flight_recompiles:recoveredInFlightRecompiles','authority_changed:false','source_bytes_moved:false','human_closure_required:true'
...
assert.match(dirtyGuard, /function recoverAfterInFlightRecompile\(event\)[\s\S]{0,260}!dirtyDraftActive[\s\S]{0,160}custodyHoldIsActive\(\)[\s\S]{0,160}!canonicalMapWorkshopIsActive\(\)[\s\S]{0,260}__td613AshA8MapReturnHandshake\?\.restore/, 'A8 must recover captured parity only after an admitted in-flight recompile completes during an active dirty Map edit.');
assert.doesNotMatch(dirtyGuard, /function recoverAfterInFlightRecompile\(event\)[\s\S]{0,220}dirtyDraftActive = false/, 'In-flight recovery must preserve the dirty edit session until the visible Commit gesture.');
