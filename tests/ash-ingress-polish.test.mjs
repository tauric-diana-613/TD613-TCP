...
assert.match(profilePrompt, /td613\.ash\.profile-prompt-canonical\/v0\.8-bounded-case-choice-remount-reconciliation/);
...
assert.match(profilePrompt, /function applyRemountedCaseChoice\(select, reason = 'REMOUNTED_CASE_SELECTION'\)/);
assert.match(profilePrompt, /function bindRemountedCaseChoice\(select, reason = 'CASE_LIST_IDENTITY_CHANGED'\)/);
assert.match(profilePrompt, /select\.addEventListener\('change', \(\) => applyRemountedCaseChoice\(select\)\)/);
assert.match(profilePrompt, /ashRemountedCaseChoiceBound/);
assert.match(profilePrompt, /openSelectedCase/);
assert.match(profilePrompt, /deleteSelectedCase/);
assert.match(profilePrompt, /operator-selected-remounted-case/);
...
assert.match(profilePrompt, /bindRemountedCaseChoice\(settled, reason\)/);
...
assert.match(bridge, /ash-profile-prompt-canonical\.js\?v=20260723-profile-prompt-v4/);
