...
assert.match(profilePrompt, /td613\.ash\.profile-prompt-canonical\/v0\.6-bounded-remount-reconciliation/);
...
assert.match(profilePrompt, /if \(reconcileQueued\) return/);
...
assert.match(profilePrompt, /currentSelect === observedSelect && currentStart === observedStart/);
assert.match(profilePrompt, /queueControlReconcile\('INGRESS_CONTROL_IDENTITY_CHANGED'\)/);
...
assert.match(bridge, /ash-profile-prompt-canonical\.js\?v=20260723-profile-prompt-v2/);
