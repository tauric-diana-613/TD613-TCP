...
import { assessMeaningPreservation } from '../app/engine/learned-audit.js';
...
console.log('  (positive spread means signal higher on preserves; negative means signal higher on breaks)');
console.log();
...
console.log('--- local Ollama audit (Dolphin3) ---');
...
console.log(`    local Ollama  F1 = ${learnedPRF1.f1.toFixed(3)}  (precision=${learnedPRF1.precision.toFixed(3)} recall=${learnedPRF1.recall.toFixed(3)} agreement=${(learnedAgreement * 100).toFixed(1)}%)`);
console.log();
console.log('  token usage across the run:');
console.log(`    input: ${totalInputTokens}  output: ${totalOutputTokens}`);
console.log();
console.log('reporting-only; does not gate npm test. labels are human-reviewed calibration labels.');
