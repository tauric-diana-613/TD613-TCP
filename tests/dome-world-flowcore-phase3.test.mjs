...
assert.match(guard,/FLOWCORE_CONTEXT_OPERATION/);
assert.match(guard,/dispatch_guarded_post/);
assert.match(guard,/flowcore_readiness_receipt/);
assert.match(guard,/MAX_BODY_BYTES = 131_072/);
...
assert.equal(fs.existsSync('api/flowcore-context.py'),false,'Phase III must not allocate a thirteenth Vercel function');
assert.equal(Object.hasOwn(vercel.functions,'api/flowcore-context.py'),false);
assert.ok(vercel.rewrites.some(route=>route.source==='/api/flowcore-context'&&route.destination==='/api/dome-world-engine-guard?operation=flowcore-context'));
