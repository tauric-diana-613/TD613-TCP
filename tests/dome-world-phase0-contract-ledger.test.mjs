...
const manifestSchema = JSON.parse(
  read('app/dome-world/schemas/ash-custody-manifest-v07.schema.json'),
);
...
assert.match(readme, /canonical local commitment module/);

assert.match(kernel, /TD613 Ash Local Commitment Kernel v0\.7\.1/);
assert.match(kernel, /selectionEpoch/);
assert.match(kernel, /invalidateLocalCommitmentSelection/);
...
for (const [name, config] of Object.entries(vercel.functions || {})) {
  if ('includeFiles' in config) {
    assert.equal(typeof config.includeFiles, 'string', `${name}.includeFiles must be a string`);
  }
  if ('excludeFiles' in config) {
    assert.equal(typeof config.excludeFiles, 'string', `${name}.excludeFiles must be a string`);
  }
}
