...
async function capture(page, base, prefix, strict) {
  ...
  if (strict && viewport.width < 600 && viewport.height > viewport.width) {
    expect(boot.y).toBeGreaterThan(viewport.height * .4);
    expect(boot.height).toBeLessThan(viewport.height * .54);
    expect(fontReady).toBe(true);
  }
  ...
  return { base, prefix, strict, boot, fontReady, computedFamily, metrics };
}
