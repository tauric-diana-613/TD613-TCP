...
const clinicalA = 'T\u0301';
const clinicalB = 'H\u0302\u0316\u0323';
const clinicalC = 'E\u0301\u0302\u0307\u030B\u0319\u031C\u0326';
const clinicalD = 'G\u0303\u0317';
const clinicalE = 'R\u0351\u0352\u0317\u0326';
...
await check('Kʰonapolit advances the human frontier after a seat-local HTTP 400 request rejection', async () => {
  ...
  try {
    ...
    assert.equal(generationCalls, 2, JSON.stringify({ attempts: res.payload?.attempts?.map(a=>({model:a.model,status:a.status,error:a.error})), status: res.statusCode, error: res.payload?.error, plan: res.payload?.modelPolicy?.callableModels, inputError: res.payload?.validation }));
