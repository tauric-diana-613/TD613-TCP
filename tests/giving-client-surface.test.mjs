...
assert.match(polish, /position: fixed !important;[\s\S]*top: 68px !important;[\s\S]*width: 100vw !important/, 'compact section rail is physically pinned below the masthead instead of remaining in ledger flow');
assert.match(polish, /@media \(max-width: 760px\)[\s\S]*\.ledger-tabs \{[\s\S]*top: 62px !important/, 'mobile rail follows the shorter mobile masthead');
assert.match(polish, /\.ledger-column \{[\s\S]*padding-top: 40px;/, 'ledger reserves space for the fixed section rail');
assert.match(polish, /\.dossier-actions #newDossierButton,[\s\S]*#saveDossierButton[\s\S]*height: 28px !important;[\s\S]*max-height: 28px !important;/, 'tablet dossier actions are hard-bounded and cannot stretch into the next field');
assert.match(polish, /@media \(max-width: 760px\)[\s\S]*#saveDossierButton[\s\S]*height: 26px !important;[\s\S]*max-height: 26px !important;/, 'mobile dossier actions stay compact despite the base 42px button minimum');
