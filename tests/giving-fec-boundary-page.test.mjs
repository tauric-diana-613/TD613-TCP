...
assert.match(pageSize, /const FEC_EXACT_BOUNDARY_PAGE_SIZE = 50;/, 'exact-name FEC retrieval must use the smaller common-name browser boundary');
assert.match(pageSize, /const PAGE_SIZE = 300;/, 'non-FEC absolute browser ceiling remains available');
assert.match(pageSize, /body\.payload\.query\.exact_match \? FEC_EXACT_BOUNDARY_PAGE_SIZE : FEC_BOUNDARY_PAGE_SIZE/, 'exact-name FEC selection must be derived from the request envelope');
...
assert.match(pagingLoader, /giving-page-size\.js\?v=20260818-2/, 'FEC/EasyVote page-size repair must use the current browser asset epoch');
...
assert.match(bootstrap, /GIVING_ASSET_EPOCH = '20260818-5'/, 'Giving coordinated module graph must use the current sealed asset epoch');
assert.match(bootstrap, /GIVING_REPAIR_EPOCH = '20260818-2'/, 'the current repair bundle must carry an independent fresh asset key');
...
console.log('giving-fec-boundary-page.test.mjs passed: FEC returns one bounded provider page per explicit gesture, exact-name retrieval uses the smaller 50-row browser boundary, continuation remains explicit, and Pedagogue route-memory stays witnessed.');
