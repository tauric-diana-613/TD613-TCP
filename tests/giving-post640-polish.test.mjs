...
const modelPrefetchIndex = bootstrap.indexOf("await fetch(epochUrl('./giving-model.js')");
const apertureIndex = bootstrap.indexOf("await import(epochUrl('./giving-aperture-context.js'))");
const bootstrapSettlementIndex = bootstrap.lastIndexOf("publishSettlement('bootstrap')");
assert.ok(shellImportIndex >= 0 && modelPrefetchIndex > shellImportIndex, 'primary Giving shell must hydrate before retrieval/model prefetch');
assert.ok(apertureIndex > shellImportIndex, 'primary Giving shell must hydrate before heavier instrumentation modules');
assert.ok(bootstrapSettlementIndex > apertureIndex, 'full bootstrap settlement must be terminal after the remaining module graph');
