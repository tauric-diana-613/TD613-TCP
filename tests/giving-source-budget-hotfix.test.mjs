...
assert.match(bootstrap, /GIVING_ASSET_EPOCH = '20260818-5'/, 'Giving must carry the current coordinated eviction epoch');
assert.match(bootstrap, /GIVING_REPAIR_EPOCH = '20260818-2'/, 'repair bundle must carry its own fresh eviction epoch');
...
assert.match(pagingEntry, /giving-page-size\.js\?v=20260818-2/, 'FEC/EasyVote-aware request normalizer must load through the current repair epoch');
...
assert.match(pageSizeModule, /FEC_BOUNDARY_PAGE_SIZE = 100/, 'broad FEC browser search must stay to one provider page per gesture');
assert.match(pageSizeModule, /FEC_EXACT_BOUNDARY_PAGE_SIZE = 50/, 'exact FEC browser search must use the smaller common-name boundary');
assert.match(pageSizeModule, /EASYVOTE_BOUNDARY_PAGE_SIZE = 50/, 'EasyVote browser search must use a source-specific response-safe boundary');
assert.match(pageSizeModule, /body\.payload\.query\.exact_match \? FEC_EXACT_BOUNDARY_PAGE_SIZE/, 'exact FEC selection must be driven by the actual request envelope');
assert.match(pageSizeModule, /sourceId\.startsWith\('easyvote-'\)/, 'EasyVote boundary must cover every registered EasyVote tenant');
