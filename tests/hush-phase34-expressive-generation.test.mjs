...
const providerContract = fs.readFileSync('server/hush-provider-contract.js', 'utf8');
...
assert(phase31Ui.includes('externalEditOwner: true'));
assert(phase31OriginalUi.includes('exportCustomMaskJson'));
assert(phase31OriginalUi.includes('importCustomMaskJson'));
...
assert(hushHtml.includes('hush-pr168-strict-transform-run-lock.js'));
assert(stableTransform.includes('hush-swap-patch38.js'));
...
assert(providerContract.includes('LAYOUT CADENCE CUSTODY'));
assert(providerContract.includes("from '../app/engine/hush-speech-act-custody.js'"));
assert(providerContract.includes('speechActPromptLaw(sourceText)'));
assert(speechActCustody.includes('SOURCE SPEECH-ACT CUSTODY LAW'));
assert(providerContract.includes('speechActCustody.passed'));
assert(proxy.includes('quarantineCandidateRows(parsed.candidates, contract)'));
assert(providerContract.includes('Line breaks and paragraph breaks are cadence evidence') || phase35Provider.includes('Line breaks and paragraph breaks are cadence evidence'));
assert(providerContract.includes('punctuation scarcity'));
assert(providerContract.includes('source_layout_cadence'));
