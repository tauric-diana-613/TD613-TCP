...
const canonicalByLabel = new Map(canonicalMasks.map((mask) => [mask.label, mask]));

assert.equal(labels.length, EXPECTED_MASK_LABELS.length);
for (const label of EXPECTED_MASK_LABELS) {
  assert.ok(labels.includes(label), `${label} missing from Phase 9 manifests`);
  assert.ok(canonicalByLabel.get(label)?.family, `${label} missing canonical family`);
}
