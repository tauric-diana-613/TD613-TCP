...
assert.equal(HUSH_SYNTAX_RECOMPOSER_VERSION, 'phase-19');
...
const text = applySyntaxOperation({ family: 'record-first', sourceText, meaningPlan, syntaxPlan });
assert(text.includes('DOC-613'));
assert(text.includes('6/13'));
assert(/not|No extra claim|review/i.test(text));

const candidate = recomposeSyntaxCandidate({ id: 'syntax-one', family: 'caveat-first', sourceText, meaningPlan, syntaxPlan });
assert.equal(candidate.id, 'syntax-one');
assert.equal(candidate.family, 'caveat-first');
assert(candidate.operations.includes('syntax-recompose'));
assert(candidate.text.includes('DOC-613'));

const bundle = generateSyntaxRecomposerCandidates({ sourceText, meaningPlan, claimRoleMap, literalPlacementMap, syntaxPlan, protectedLiterals, candidateCount: 18 });
assert.equal(bundle.version, 'phase-19');
assert(bundle.candidates.length >= 12);
assert(bundle.candidates.every((item) => item.text.includes('DOC-613') && item.text.includes('6/13')));
assert(bundle.candidates.every((item) => /not|No extra claim|review|narrow/i.test(item.text)));
