...
test('provider failover receipt keeps at most three finite model and status pairs', () => {
  const input = payload();
  ...
  input.observations = {
    model: 'gemini-2.5-flash', provider_calls: 3, http_status: 503,
    provider_attempts: [
      { model: 'gemini-3.8-flash', status: 503 },
      { model: 'gemini-3.5-flash', status: 503 },
      { model: 'gemini-2.5-flash', status: 503 }
    ]
  };
  const failure = readLoomAiFailure(input, 'request-1');
  ...
  for (const invalid of [
    ...
    [{ model: 'a', status: 503 }, { model: 'b', status: 503 }, { model: 'c', status: 503 }, { model: 'd', status: 503 }],
