...
assert.match(source, /marrowlinePrimaryDiagnostic\?\.code === 'PROVIDER_RATE_LIMIT_HELD'/);
assert.match(source, /marrowlinePrimaryRateLimitScopes\.length > 0/);
assert.match(source, /marrowlinePrimaryRateLimitScopes\.every\(scope => scope === 'model'\)/);
...
assert.match(source, /'model-rate-limit-held'/);
