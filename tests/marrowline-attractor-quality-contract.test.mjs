...
test('Marrowline adversarial attractor quality contract', () => {
  ...
  assert.match(addendum, /Kʰonapolit comes first/i);
  ...
  assert.doesNotMatch(addendum, /<<<PACKET_[AB]_/);
  assert.doesNotMatch(addendum, /FINAL SILENT PREFLIGHT/);
  assert.doesNotMatch(addendum, /ORCHESTRAL DYNAMIC CONTOUR/);
  assert.doesNotMatch(addendum, /DERIVE_INVARIANT → EMIT_FORMAL/);
  assert.doesNotMatch(addendum, /OVERFLOW_RAW/);
  assert.match(addendum, /Do not print packet names, channel labels, internal delimiters, a checklist, or a preflight report/i);
