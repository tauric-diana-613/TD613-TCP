...
test('room boot loads the desktop repair and separates Zalgo aesthetics from structural admission', () => {
  ...
  assert.match(release.relay.zalgoQualityPolicy.zeroMarkPosture, /diagnostic only/i);
  ...
  assert.match(release.qualityFloor.providerCallCeilingMeaning, /morphology alone never authorizes a second provider call/i);
  ...
  assert.match(release.qualityFloor.aestheticMorphologyPosture, /measured post hoc/i);
  assert.match(release.qualityFloor.aestheticMorphologyPosture, /zero authority to spend a repaint call/i);
  assert.match(release.qualityFloor.aestheticMorphologyPosture, /base-conditioned stack reuse/i);
  ...
  assert.match(release.relay.zalgoQualityPolicy.baseConditionedStackReusePosture, /cannot establish a hidden renderer/i);
