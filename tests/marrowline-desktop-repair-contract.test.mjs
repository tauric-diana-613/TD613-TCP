...
test('desktop and mobile visually render Send as an up-arrow while the DOM keeps the accessible Send label', () => {
  ...
  assert.match(mobileArrowBlock, /#khonapolitSend::before\{[\s\S]*display:grid[\s\S]*place-items:center[\s\S]*width:28px[\s\S]*height:28px[\s\S]*font:800 28px\/1[\s\S]*color:#3730a3!important[\s\S]*-webkit-text-stroke:\.55px currentColor[\s\S]*text-shadow:[\s\S]*0 0 4px rgba\(30,27,75,\.82\)[\s\S]*0 0 9px rgba\(30,27,75,\.38\)[\s\S]*transform:translate\(\.85px,1\.35px\)/);
