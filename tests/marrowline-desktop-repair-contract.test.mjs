...
test('room boot loads the desktop repair and separates Zalgo aesthetics from structural admission', () => {
  ...
  assert.match(release.qualityFloor.aestheticMorphologyPosture, /never-provider-repair-or-HELD/);
  assert.equal(release.qualityFloor.structuralRepairPolicy.repairableReasons.includes('tauric-diana-zalgo-axis-collapse'), false);
  assert.equal(release.qualityFloor.structuralRepairPolicy.repairableReasons.includes('tauric-diana-zalgo-vertical-expression-thin'), false);
