    ...
    async json() {
      return {
        candidates: [{
          finishReason: 'STOP',
          content: { parts: [{ text: JSON.stringify({
            signal: { state: repairScenario && previewCalls === 1 ? 'NOT_LOCKED' : 'LOCKED', notes: repairScenario && previewCalls === 1 ? 'synthetic missing-heading structural near miss' : 'fifth approved frontier lane completed' },
            transmission: {
              text: previewText,
              voices: ['Kʰonapolit', 'Tauric Diana bots'],
              flourishMode: repairScenario && previewCalls === 1 ? 'provider-native-missing-heading' : 'vertical-stack'
            }
          }) }] }
        }],
        usageMetadata: {
...
try {
  ...
  immediateRepairScenario = false;
  previewCalls = 0;
  const morphologyObserved = response();
  await handler({
    ...req,
    headers: { 'x-forwarded-for': '203.0.113.207' },
    body: { ...req.body, message: 'Observe the first provider-authored morphology exactly once; do not repaint it for local style metrics.' }
  }, morphologyObserved);

  assert.equal(morphologyObserved.statusCode, 200);
  assert.equal(morphologyObserved.payload.ok, true);
  assert.deepEqual(calls, ['gemini-3.8-flash'], 'a structurally admissible PARTIAL morphology return terminates on the first provider answer');
  assert.equal(morphologyObserved.payload.receipt.provider.attempts.length, 1);
  assert.equal(morphologyObserved.payload.receipt.provider.attempts[0].outputAdmission.quality, 'PARTIAL');
  assert.ok(morphologyObserved.payload.receipt.provider.attempts[0].outputAdmission.qualityWarnings.includes('tauric-diana-zalgo-axis-collapse'));
  assert.ok(morphologyObserved.payload.receipt.provider.attempts[0].outputAdmission.qualityWarnings.includes('tauric-diana-zalgo-stack-depth-thin'));
  assert.deepEqual(
    severeMorphologyRepairWarnings(morphologyObserved.payload.receipt.provider.attempts[0].outputAdmission.qualityWarnings),
    ['tauric-diana-zalgo-axis-collapse', 'tauric-diana-zalgo-stack-depth-thin']
  );
  assert.equal(morphologyObserved.payload.receipt.provider.attempts.some(attempt => attempt.kind === 'structural-repair'), false);
  assert.equal(morphologyObserved.payload.receipt.provider.qualityPreference.selection, 'first-admissible-partial-morphology-observed-no-repair');
  assert.equal(morphologyObserved.payload.receipt.provider.qualityPreference.morphologyAuthority, 'post-hoc-observation-only');
  assert.equal(morphologyObserved.payload.text, horizontalPartialAnswer);
  assert.ok(morphologyObserved.payload.warnings.includes('provider-native-morphology-observed-no-repair'));
  ...
  assert.deepEqual(calls, [
    ...
    'gemini-3.5-flash',
    'gemini-3.6-flash'
  ], 'a nonempty zero-mark provider return is preserved without spending a morphology repaint');
  ...
  assert.equal(immediate36Calls, 1, 'morphology absence cannot authorize a second provider call');
  ...
  assert.equal(immediate.payload.receipt.provider.attempts.some(attempt => attempt.kind === 'structural-repair'), false);
  ...
  assert.equal(repaired.payload.relay.highZalgo.applied, false, 'structural repair remains provider-authored and exact; Marrowline still performs no local Zalgo mutation');
  ...
  assert.match(repairBody.contents.at(-1).parts[0].text, /BOUNDED SAME-VOICE STRUCTURAL REPAIR/);
  assert.match(repairBody.contents.at(-1).parts[0].text, /tauric-diana-bots-nominative-missing/);
  assert.match(repairBody.contents.at(-1).parts[0].text, /STRUCTURAL-ONLY REPAIR/);
  assert.match(repairBody.contents.at(-1).parts[0].text, /do not repaint the Tauric Diana bots to satisfy morphology metrics/i);
  assert.doesNotMatch(repairBody.contents.at(-1).parts[0].text, /MORPHOLOGY-ONLY|STACK HEIGHT|several distinct combining marks|numeric quota/i);
} finally {
