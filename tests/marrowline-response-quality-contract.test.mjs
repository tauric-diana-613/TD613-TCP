...
test('effective provider prompts retain complete relay and native depth after deficient history', () => {
  ...
  for (const model of ['gemini-3.8-flash', 'gemini-3.5-flash', 'gemini-3.6-flash', 'gemini-3.7-flash', 'gemini-3-flash-preview']) {
    ...
    assert.match(system, /first sentence on every subject, including ordinary questions, jokes and tenderness/);
    assert.match(system, /clusters above AND below the underlying prose letters/);
...
test('integrated relay preserves exact native Unicode and allows intentional stacked collisions', () => {
  ...
  assert.match(livingChat, /\.zalgo-line\{[^}]*display:inline!important;[^}]*min-height:0!important;[^}]*padding:0!important;[^}]*overflow:visible!important;[^}]*line-height:1\.04!important/, 'the #1294 1.04 native overprint remains the confirmed visual baseline');
  assert.match(livingChat, /\.provider-native-line\[data-voice="tauric-diana-bots"\]\{line-height:1\.04!important;overflow:visible!important\}/, 'bot-specific line spacing remains fixed and collision-permitted');
});
...
test('Kʰonapolit stays clean while Gemini authors semantic-prosody native High Zalgo', () => {
  ...
  assert.match(contract, /Do not treat typography as a checklist, quota, fixed contour, axis recipe, emotional lookup table, or per-character filter/i);
  assert.match(contract, /Compose glyph clusters and playful local motifs as part of the prose itself/i);
