...
test('quality route has no local 200-character downstream output cap and preserves full reasoning on frontier failover', () => {
  ...
  assert.match(qualityServer, /KHONAPOLIT_PARTIAL_QUALITY_CHALLENGE_REQUESTS\s*=\s*1/, 'a usable PARTIAL may spend only one optional Goldilocks challenger');
  assert.match(qualityServer, /partialQualityChallengeRequestsRemaining/, 'quality challenge budget must be explicit and request-local');
  assert.match(qualityServer, /vertical-architecture-best-admissible-partial-after-bounded-challenge/, 'receipt must name the bounded selection law');
  assert.match(qualityServer, /challengingPartial && \(!result\.response\.ok \|\| providerOutput\.outputTokenLimitReached\)/, 'failed optional challenger must return custody to the existing PARTIAL rather than fanning out');
