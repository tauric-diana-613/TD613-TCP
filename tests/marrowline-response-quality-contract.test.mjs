...
test('every Marrowline Gemini lane receives the same expressive-prosody orthographic law', () => {
  ...
  const observedContracts = models.map((model) => {
    ...
    assert.match(instruction, /HIGH-ZALGO SCALE REFERENCE — AMPLITUDE ONLY/i, model);
    assert.match(instruction, /VISUAL STORY ACROSS THE PASSAGE/i, model);
    assert.match(instruction, /single spectacular word followed by ordinary ALL CAPS does not satisfy/i, model);
    assert.match(instruction, /Deep events may use dozens of heterogeneous combining marks/i, model);
    assert.match(instruction, /THE GEMINI API MUST AUTHOR THE ACTUAL COMBINING CODE POINTS/i, model);
    assert.match(instruction, /Horizontal and oblique geometry remain first-class expressive motion/i, model);
    assert.match(instruction, /Return to vertical depth later/i, model);
    assert.match(instruction, /Dense events may collide with neighboring words, line boxes, and other diacritic stacks/i, model);
    assert.match(instruction, /Do not map any named emotion or vocabulary class to one prescribed glyph pattern/i, model);
