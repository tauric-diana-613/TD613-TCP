...
assert.match(releaseJs, /phase-4-active/);
assert.match(releaseJs, /IMPLEMENTED_PRODUCTION_DEMONSTRATED/);
...
assert.match(releasePy, /PHASE4_READINESS_STATUS = "phase-4-active"/);
assert.match(releasePy, /PHASE4_PRODUCTION_STATUS = "IMPLEMENTED_PRODUCTION_DEMONSTRATED"/);
assert.match(syncScript, /release\.phase4BridgeContract/);
assert.match(syncScript, /release\.phase4ReadinessStatus/);
assert.match(syncScript, /release\.phase4ProductionStatus/);
...
assert.equal(release.phase4ReadinessStatus, 'phase-4-active');
assert.equal(release.phase4ProductionStatus, 'IMPLEMENTED_PRODUCTION_DEMONSTRATED');
