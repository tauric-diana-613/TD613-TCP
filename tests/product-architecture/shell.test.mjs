...
assert.match(renderedKeep,new RegExp(`dataset\\.ashAia3='${escaped(ASH_AIA3_COMPOSITION_VERSION)}'`));
...
for(const pattern of [/caseMapDigest: state\.caseMap\.case_map_digest/,/releaseReceiptReference: state\.latestRelease\?\.receipt_id \|\| null/,/releaseReceiptDigest: state\.latestRelease\?\.receipt_digest \|\| null/,/latestSavePoint\.release_receipt_reference !== currentRelease\.receipt_id/,/EVENT_DRIVEN_COALESCED/,/function requestMapDraw\(\)/])assert.match(renderedKeepJs,pattern);
...
const rewrites=vercel.rewrites;const genericIndex=rewrites.findIndex(rule=>rule.source==='/dome-world/(.*)');
for(const [source,destination] of [['/dome-world/ash-threshold.html','/api/dome-world-shell?surface=ash-keep-html'],['/dome-world/ash-keep.html','/api/dome-world-shell?surface=ash-keep-html'],['/dome-world/ash-keep.js','/api/dome-world-shell?surface=ash-keep-js']]){const index=rewrites.findIndex(rule=>rule.source===source&&rule.destination===destination);assert.ok(index>=0&&index<genericIndex,`${source} must precede the generic Dome route`);}
