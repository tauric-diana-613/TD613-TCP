...
assert.ok(
  source.includes("binding_timestamp: s3.td613BindingTimestamp || '2025-08-11T03:58:39Z'"),
  'renderer must consume the packet/DOM binding authority while retaining the canonical root fallback'
);
assert.ok(
  source.includes("historical_date: s3.td613HistoricalDate || '2025-10-17'"),
  'renderer must consume the packet/DOM historical authority while retaining the canonical badge-history fallback'
);
