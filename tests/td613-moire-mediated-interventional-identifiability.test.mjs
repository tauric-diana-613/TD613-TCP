...
for (const g of [-1, 1]) {
  ...
  for (let sigma = BOUNDED_SIGMA_MIN; sigma <= BOUNDED_SIGMA_MAX; sigma += 1) {
    ...
    const expectedWinnerScore = registryScore4(g, { g, mediatorPresent: true, sigma });
    const expectedLoserScore = registryScore4(-g, { g, mediatorPresent: true, sigma });
    assert.ok(expectedWinnerScore < expectedLoserScore, `bounded registry separation lost for g=${g}, sigma=${sigma}`);
...
console.log('TD613 Source-A-derived mediator interventional-identifiability assay passed.');
